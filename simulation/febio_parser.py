...
This module also provides lightweight utilities for reading FEBio node
coordinates without importing DOLFINx/mpi4py (useful for analysis scripts).
"""
...
def _parse_febio_xyz(text: str) -> np.ndarray:
    raw = text.strip()
    if not raw:
        raise ValueError("Empty FEBio node coordinate text")
    if "," in raw:
        xyz = np.fromstring(raw, sep=",", dtype=float)
    else:
        xyz = np.fromstring(raw, sep=" ", dtype=float)
    if xyz.size != 3 or not np.isfinite(xyz).all():
        raise ValueError(f"Invalid FEBio node coordinate triplet: '{raw}'")
    return xyz


def parse_febio_node_coords(mesh_xml: ET.Element) -> np.ndarray:
    """Return node coordinates ordered by ascending FEBio node id.

    This ordering matches :class:`FEBio2Dolfinx` after its internal reindexing
    (sorted old ids → contiguous 1..N).
    """
    node_elems = mesh_xml.findall(".//Nodes/node")
    if not node_elems:
    ...
    for node in node_elems:
        node_id = node.get("id")
        if node_id is None:
            raise ValueError("FEBio <node> missing 'id' attribute")
        text = node.text
        if text is None:
            raise ValueError(f"FEBio <node id='{node_id}'> has no coordinate text")
        ids.append(int(node_id))
        coords.append(_parse_febio_xyz(text))

    ids_arr = np.asarray(ids, dtype=np.int64)
    if ids_arr.size != int(np.unique(ids_arr).size):
        unique, counts = np.unique(ids_arr, return_counts=True)
        dup = unique[counts > 1]
        raise ValueError(f"Duplicate FEBio node ids found: {dup.tolist()}")

    order = np.argsort(ids_arr)
    out = np.asarray([coords[i] for i in order], dtype=float)
    return out


def load_febio_node_coords(feb_file: str | Path) -> np.ndarray:
    """Load FEBio node coordinates from a .feb file.

    Returns
    -------
    coords : (n_nodes, 3)
        Node coordinates in ascending id order (contiguous 0..n_nodes-1 indexing).
    """
    feb_path = Path(feb_file)
    tree = ET.parse(str(feb_path))
    root = tree.getroot()
    mesh_xml = root.find("Mesh")
    if mesh_xml is None:
        raise ValueError("Missing <Mesh> section in FEBio file")
    return parse_febio_node_coords(mesh_xml)
...
class FEBio2Dolfinx:
    ...
    def _extract_geometry(self) -> None:
        ...
        coords = parse_febio_node_coords(self.mesh_xml)
