...
def init():
    global TEMPLATE, DOFS, SHAPES, STORES, CFG
...
def subject(i):
    ...
    for ch in CHANNELS:
        ...
        if ch=='material_only': phi=np.zeros_like(coords)
            ...
            phi=RBFInterpolator(TEMPLATE.points,SHAPES[i]-TEMPLATE.points,
                smoothing=cfg['RBF_DATA_SMOOTHING'],neighbors=cfg['RBF_DATA_NEIGHBORS'])(coords)
        ...
        surface=RBFInterpolator(coords,fields,smoothing=cfg['RBF_SOLVER_SMOOTHING'],
                               neighbors=cfg['RBF_SOLVER_NEIGHBORS'])(TEMPLATE.points)
