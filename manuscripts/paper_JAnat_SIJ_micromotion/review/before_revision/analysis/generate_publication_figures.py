...
def build_figure_1() -> None:
    ...
    # --- Section 1: Rotations ---
    ax_B2.text(0.04, 0.950, r"$\mathbf{3\;Rotational\;DOFs}$ (Cardan $XYZ$ Order):", transform=ax_B2.transAxes,
    ...
    ax_B2.text(0.09, 0.880, r"$\mathbf{\theta_{\mathrm{nut}}}$ ($\alpha_x$, ML axis): $\mathbf{Nutation}$",
               transform=ax_B2.transAxes, fontsize=7.4, fontweight="bold", color=COLOR_ML, va="center")
    ax_B2.text(0.09, 0.825, r"+ Anterior sacral tilt (self-bracing) / − Counternutation",
    ...
    ax_B2.text(0.09, 0.750, r"$\mathbf{\theta_{\mathrm{abd}}}$ ($\beta_y$, AP axis): $\mathbf{Out\text{-}flare / Abduction}$",
               transform=ax_B2.transAxes, fontsize=7.4, fontweight="bold", color=COLOR_AP, va="center")
    ax_B2.text(0.09, 0.695, r"+ Lateral iliac wing opening / − In-flare (adduction)",
               transform=ax_B2.transAxes, fontsize=6.6, color="#334155", va="center")
    ...
    ax_B2.text(0.09, 0.565, r"+ Internal axial rotation / − External axial rotation",
               transform=ax_B2.transAxes, fontsize=6.6, color="#334155", va="center")
    ...
    ax_B2.text(0.09, 0.408, r"$\mathbf{d_{\mathrm{ML}}}$ ($dx$, ML): $\mathbf{Distraction / Compression}$",
               transform=ax_B2.transAxes, fontsize=7.4, fontweight="bold", color=COLOR_ML, va="center")
    ax_B2.text(0.09, 0.353, r"+ Joint gapping (distraction) / − Joint compression",
               transform=ax_B2.transAxes, fontsize=6.6, color="#334155", va="center")
    ...
    ax_B2.text(0.09, 0.278, r"$\mathbf{d_{\mathrm{AP}}}$ ($dy$, AP): $\mathbf{Anteroposterior\;Gliding / Shear}$",
               transform=ax_B2.transAxes, fontsize=7.4, fontweight="bold", color=COLOR_AP, va="center")
    ax_B2.text(0.09, 0.223, r"+ Anterior gliding / − Posterior gliding",
               transform=ax_B2.transAxes, fontsize=6.6, color="#334155", va="center")
    ...
    ax_B2.text(0.09, 0.148, r"$\mathbf{d_{\mathrm{CC}}}$ ($dz$, CC): $\mathbf{Vertical\;Craniocaudal\;Shear}$",
               transform=ax_B2.transAxes, fontsize=7.4, fontweight="bold", color=COLOR_CC, va="center")
    ax_B2.text(0.09, 0.093, r"+ Cranial displacement / − Caudal displacement",
               transform=ax_B2.transAxes, fontsize=6.6, color="#334155", va="center")
    ...
    ax_hdr_stand.text(0.50, 0.50, "Habitual Locomotor Regimes (Standing)", transform=ax_hdr_stand.transAxes,
    ...
    card_data = [
        {
            ...
            "response": r"Self-bracing nutation ($\theta_{\mathrm{nut}} \approx 1.06^\circ$)",
        ...
        },
        {
            ...
            "response": r"Asymmetry jump ($\Delta_{\mathrm{asym}} \approx 2.01$ mm)",
            "bg": "#fffbeb", "border": "#fde68a", "accent": "#b45309"
        },
        {
            "title": "Inlet Compression Proxy",
            "force": r"$\pm 400$ N lateral pair",
            "target": "Inner pectineal ring",
            "response": r"Transverse compliance ($\Delta\mathrm{ML} = +0.09$ mm)",
            "bg": "#fdf2f8", "border": "#fbcfe8", "accent": "#be185d"
        },
        {
            "title": "Tuberosity Distraction Proxy",
            "force": r"$\pm 400$ N lateral pair",
            "target": "Ischial tuberosities",
            "response": r"Max outward expansion ($\Delta\mathrm{ML} = +0.15$ mm)",
            "bg": "#faf5ff", "border": "#e9d5ff", "accent": "#7c3aed"
        },
        {
            "title": "Outlet AP Distraction Proxy",
            "force": r"$\pm 400$ N AP pair",
            "target": "Pubis & SCJ",
            "response": r"Sagittal rotation ($|\mathbf{\theta}| = 1.34^\circ$)",
            "bg": "#fffbeb", "border": "#fde68a", "accent": "#b45309"
        },
    ]
