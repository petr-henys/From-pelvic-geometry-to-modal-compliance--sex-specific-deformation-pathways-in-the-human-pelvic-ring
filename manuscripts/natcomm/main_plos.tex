...
\newcommand{\ManuscriptTitle}{Stable modal subspaces preserve inlet-related deformation across human pelvic anatomical variability}
\newcommand{\ShortTitle}{Stable modal subspaces in the human pelvis}
...
\vspace{1.5em}
{\normalsize Petr Heny\v{s}$^{1,2}$, Niels Hammer$^{2,3,4,*}$\par}
\vspace{0.75em}
...
% ============================================================
\section*{Abstract}
Human pelvis kinematics must be maintained across marked variation in ring geometry, inlet and outlet dimensions, as well as bone architecture.
...
Here, the human pelvis has been utilized to assess whether key mechanical functions remain attached to individually stable pathways or are preserved only within small recurrent groups of interchangeable pathways.
...
Across the population cohort, pelvic mechanics can be separated into two distinct regimes.
Modes~1--3 form a gap-protected low-rank \emph{backbone}, which rarely reorders across subjects ($<0.4\%$ swap rate) and dominates standing loads.
Higher ranks contain recurrent \emph{clustered} regions, most prominently an inlet-related 9--10 block present in 54.7\% of individuals.
...
These results indicate that pelvic mechanical function is organised across spectral scales: some features remain tied to stable low-rank pathways, whereas others are preserved only within the smallest recurrent function-preserving subspace.
More generally, the analyses offered an approach to identify how biological mechanical function is maintained in anatomically variable structures without assuming one-to-one preservation of spectral labels.

% ============================================================
\section*{Author summary}

Individuals differ substantially in pelvic shape and bone structure, yet the human pelvis still has to perform the same basic mechanical tasks.
We asked whether those tasks are carried by one fixed deformation pattern for each individual, or by a small set of similar patterns which can substitute for one another.
Using finite-element models of 278 pelves, we compared how deformation patterns were organised across a population subset.
It was found that everyday load transfer relies on a small low-rank backbone shared across subjects.
In contrast, some higher-order functions, including opening of the pelvic inlet, were not tied to one fixed deformation pattern.
Instead, they were preserved within a small group of nearby patterns, even when the individual labels inside that group change order between subjects.
...
Some biological structures must maintain identical functions across individuals which differ in size, shape, and tissue composition.
In load-bearing anatomy, that problem is especially clear as the same biological tasks must be met despite substantial variation in geometry and material distribution.
...
The relevant unit is a compact recurrent subspace, namely a small externally separated block that preserves a targeted function across subjects.
...
Therefore, the following objective has been studied: under subject-specific bony shape and bone elasticity, which pelvic mechanical functions remain carried by individually stable pathways, and which are preserved only within small recurrent clusters of interchangeable pathways?
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth,height=0.80\textheight,keepaspectratio]{../../results/ref_S1P_fixed_new2/modes_visualisation/modes_median_AP.png}
  \caption{\textbf{Interpretable deformation pathways preview the spectrum's two organisational scales.}
  Median displacement fields across the cohort ($N{=}278$) are shown in anterior--posterior view with superimposed deformed wireframes.
  Colour scale indicates displacement magnitude from blue (minimum/fixed S1 facet) to red (maximum nodal displacement).
  ...
  Modes~9--10 preview a compact clustered case where inlet opening is preserved at the subspace level rather than by a fixed rank label.}
...
Bootstrap resampling of random 80\% subcohorts yields narrow 95\% subsampling stability intervals for the dominant clusters (9--10: 51.8--57.7\%; 12--15: 38.7--45.0\%), and full subject bootstrap with replacement confirms consistent prevalence estimates. Random permutation of subject order leaves cluster prevalence and swap statistics unchanged to machine precision.
...
Its median AP-inlet coupling is 0.52\,mm of inlet diameter change per 1\,mm of maximum nodal modal amplitude, making 9--10 the clearest higher-rank inlet-related program in the cohort.
In contrast, the rank~5--6 cluster shows the strongest outlet-related coupling, with median biischiadic widening 1.41\,mm/mm and a smaller bituberous response (0.24\,mm/mm).
...
Among subjects within the 9--10 cluster, AP-inlet coupling attached to \emph{rank mode~9} is high when labels do not swap and low when they do (median 0.454 vs.\ 0.139\,mm/mm; difference $-0.315$\,mm/mm, 95\% CI [$-0.360$, $-0.262$], $p = 2.7\times 10^{-23}$, rank-biserial $r_{\rm rb} = -0.97$).
The complementary pattern appears for \emph{rank mode~10} (median 0.163 vs.\ 0.436\,mm/mm; difference $+0.273$\,mm/mm, 95\% CI [$0.225$, $0.335$], $p = 1.7\times 10^{-22}$, $r_{\rm rb} = 0.95$).
...
By contrast, the \emph{subspace-level} AP coupling of the 9--10 block remains substantial in both groups and shifts only modestly (median 0.523 vs.\ 0.509\,mm/mm; difference $-0.014$\,mm/mm, 95\% CI [$-0.074$, $0.012$], ratio 0.97, $p = 0.083$, $r_{\rm rb} = -0.17$).
...
\begin{figure}[!htbp]
  ...
  Anteroposterior (AP) inlet coupling for rank mode~9 \textbf{(A)}, rank mode~10 \textbf{(B)}, and the canonical two-dimensional 9--10 subspace \textbf{(C)}, stratified by $9\rightleftarrows 10$ label-exchange status.
  Points represent individual subjects (blue: male, $n{=}95$; yellow: female, $n{=}57$ across cluster members).
...
\begin{figure}[!htbp]
  ...
  \textbf{(A--C)} Modal strain-energy fraction carried by the 9--10 block as a function of AP inlet diameter for \LABone\ (bilateral ring compression), \LABtwo\ (ischial distraction), and \LABthree\ (outlet distraction).
  Blue dots and dashed lines indicate males; yellow dots and dashed lines indicate females.
  \textbf{(D)} Canonical AP-inlet coupling of the 9--10 subspace stratified jointly by sex and label-exchange status (M/no, F/no, M/yes, F/yes). Boxes show median and IQR; whiskers extend to $1.5\times\mathrm{IQR}$.}
  \label{fig:tradeoff}
\end{figure}
...
We note that a permutation null test across the empirical cohort did not show statistically significant swap localization to AP diameter alone (Table~A6), indicating that the toy model serves as a mechanistic illustration of how parameter-driven detuning across a near-degenerate zone generates veering, rather than as a proof of one-dimensional anatomical causality.
...
This given pelvic cohort shows that mechanical function can remain robust across substantial anatomical variability without preserving a one-to-one mapping between function and spectral rank.
...
This distinction separates three objects which are often treated as if they were interchangeable: anatomical shape, deformation pathway, and spectral order.
Shape can vary without preserving a label, and labels can exchange without erasing a function.
In musculoskeletal biomechanics, a classic challenge is explaining how structures with pronounced morphological divergence can perform identical kinematic and load-bearing tasks across a population.
When analysis is restricted to fixed eigenvector indices, subject-to-subject variability can easily be mistaken for functional degradation or chaotic loss of organisation.
By viewing the system through the lens of invariant modal subspaces, we demonstrate that mechanical meaning is not lost; rather, the appropriate unit of biological organisation shifts from the isolated eigenmode to a compact, recurrent subspace.
...
Modes~1--3 remain spectrally separated, rarely reorder across the cohort ($<0.4\%$ swap rate), and dominate habitual standing loads (carrying, for instance, $84.5\%$ of retained modal strain energy in symmetric two-leg stance).
Within the present linear framework, this supports the view that the primary kinematic pathways of the pelvic ring---torsion about the craniocaudal axis, sagittal nutation/counternutation, and lateral obliquity---are governed by a conserved, low-cost baseline program.
We do not infer an evolutionary or optimisation principle from these data alone, but the result is consistent with biological robustness being concentrated in a compact, gap-protected foundation~\citep{Kitano2007,Whitacre2010}.

The higher ranks reveal an entirely different organisation characterised by recurrent clustered blocks.
The rank~9--10 pair exemplifies this regime: inlet-related dilation is not reliably attached to a single rank label across the cohort, but the shared two-dimensional subspace retains substantial AP-inlet coupling ($0.523$ vs.\ $0.509$\,mm/mm between non-swapped and swapped groups) and is strongly recruited under parturition-aligned proxy loads (\LABone).
This constitutes direct physical evidence for biological degeneracy in structural mechanics: structurally distinct internal pathways can provide interchangeable functional outputs without requiring exact mathematical multiplicity~\citep{Whitacre2010,Whitacre2012,EdelmanGally2001}.
In parallel, the 5--6 block preserves a specific outlet-widening program (dominated by biischiadic diameter change), whereas the broader 12--15 family behaves as a higher-order reserve with diffuse canonical coupling.
Higher-rank pelvic mechanics is therefore not a featureless noise floor; it contains both functionally focused and diffuse subspaces.

\subsection*{Implications for the obstetrical dilemma and population biomechanics}

The human pelvic ring has long been studied through the framework of the ``obstetrical dilemma''~\citep{Washburn1960,Gruss2015,Kriechling2022}, which posits an evolutionary trade-off between the structural rigidity necessary for efficient bipedal locomotion and the internal dimensions required for the birth of large-brained neonates.
Our modal analysis provides a mechanistic perspective on how this trade-off is organized dynamically.
Locomotor loads (such as standing and single-leg support) are routed almost exclusively through the stiff, order-locked backbone (modes~1--3), shielding the ring from excessive distortion during everyday weight-bearing.
Conversely, parturition-motivated loading conditions bypass the backbone and selectively recruit higher-rank clustered modes, specifically the inlet-dilating 9--10 subspace.
...
This indicates that the pelvic ring can tolerate individual-level variations in soft-tissue stiffness and shape while reliably preserving the capacity for inlet and outlet expansion.
...
Nonetheless, our first-order sensitivity analyses (Appendix~D) confirm that realistic material uncertainty raises the overall spectral variability without reorganising the underlying gap architecture or dismantling the clustered regimes.
Extending this subspace framework to include geometrically nonlinear deformation and individualised soft-tissue kinematics represents a promising direction for future population biomechanics.
...
\begin{equation}
\mathrm{Var}(\lambda_{i+1}-\lambda_i) = \mathbf g_{i,i+1}^\top\Sigma\,\mathbf g_{i,i+1} = \mathrm{Var}(\lambda_{i+1}) + \mathrm{Var}(\lambda_i) - 2\,\mathrm{Cov}(\lambda_i, \lambda_{i+1}),
\label{eq:gap-cov}
\end{equation}
