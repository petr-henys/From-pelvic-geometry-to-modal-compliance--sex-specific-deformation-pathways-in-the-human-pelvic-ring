...
\newcommand{\ManuscriptTitle}{Stable modal subspaces retain inlet-related deformation across human pelvic anatomical variability}
...
Here, we investigate whether pelvic deformation patterns remain attached to individually stable pathways or are preserved within compact recurrent groups of interchangeable pathways.
...
An outlet-related 5--6 block similarly preserves a coherent biischiadic widening program despite local label instability.
...
We found that everyday standing loads rely on a few dominant, stable deformation patterns that remain virtually identical in order and shape across all subjects.
...
Instead, this anatomical opening behavior is preserved across a pair of interchangeable patterns whose internal order frequently swaps between individuals.
Comparing skeletons strictly mode-by-mode can therefore create the false impression of functional disruption when the underlying anatomical deformation is actually preserved.
...
The human pelvis exemplifies this challenge: it transfers locomotor forces between the spine and lower limbs while preserving birth canal dimensions critical for parturition~\citep{Washburn1960,Gruss2015,HenyssKuchar2025BoneDat,Kriechling2022}.
...
Different pelves may achieve equivalent compliance through slightly different, interchangeable deformation pathways, analogous to motor synergy recruitment across variable musculoskeletal anatomy~\citep{Ting2012}.
...
The complete mathematical formulation, four-point tetrahedral quadrature proofs, 23-dimensional polynomial basis, Gram matrix conditioning, synthetic validation benchmarks, and complete 15-mode quantitative profile tables are detailed in Appendix~E (Tables~E1--E2).
...
\begin{figure}[p]
  ...
  Anterior (left) and cranial (right) views show displacement magnitude from purple to yellow, scaled to a 14~mm maximum; grey geometry is undeformed.
...
The modest $\approx 2.7\%$ difference between the non-swapped ($n{=}95$) and swapped ($n{=}57$) groups falls well within pre-specified anatomical equivalence bounds with a 95\% confidence interval spanning zero, in stark contrast to the massive, statistically decisive redistribution seen at the single-mode level ($p < 10^{-22}$).
...
The two-dimensional cluster in the toy model plays the same role as $\mathrm{span}(Q_s)$ in the FE framework, and the morphology coordinate~$\mu$ illustrates how a continuous anatomical shift can detune eigenvalues across a crossing point.
...
A concrete practical implication follows for population biomechanics: when comparing inlet compliance across individuals using rank mode~9 alone, subjects who undergo a $9\rightleftarrows 10$ label exchange ($37.5\%$ of cluster members) exhibit an apparent collapse of AP-inlet coupling (from median $0.454$ down to $0.139$\,mm/mm, an apparent drop of $\approx 69\%$), falsely suggesting a severe impairment of ring dilation.
In reality, evaluating the shared two-dimensional 9--10 subspace reveals that the anatomical opening program is fully preserved ($0.523$ vs.\ $0.509$\,mm/mm, a difference of only $2.7\%$).
...
Within the present linear framework, this supports the view that the low-order kinematic pathways of the pelvic ringare governed by a conserved, low-cost baseline program.
...
Rather than positing an unverified evolutionary selection for structural degeneracy, our results provide a physical interpretation compatible with mechanics: in complex ring structures with near-degenerate eigenvalue spectra, degeneracy arises naturally as a consequence of three-dimensional ring geometry and material heterogeneity, enabling structurally distinct internal pathways to provide interchangeable functional deformations without requiring exact mathematical symmetry~\citep{Whitacre2010,Whitacre2012,EdelmanGally2001}.
...
\end{equation}
defining an $L^2$ volume inner product weighted by local volumetric dilation rather than physical dynamic mass density; the resulting generalised eigenvalues $\lambda_k$ therefore describe linearised static compliance rather than physical resonance frequencies.
...
\end{equation}
with measurement vectors $\mathbf b_{\rm AP}^{(s)},\mathbf b_{\rm ML}^{(s)}\in\mathbb R^n$ constructed using inverse-distance weighting (IDW) interpolation onto the $k_{\rm idw}=8$ nearest mesh nodes around each anatomical landmark coordinate, ensuring smooth, mesh-independent displacement evaluations.
...
Because an internal orthogonal basis rotation $\widetilde Q_s = Q_s R$ ($R\in\mathrm O(r)$) transforms $C_s$ covariantly as $\widetilde C_s = R^\top C_s$, the singular values $\Sigma_s$ and the physical canonical displacement fields $\boldsymbol\psi_k^{(s)}$ are strictly invariant, providing a physically stable coordinate system independent of internal basis parametrization.
Canonical coupling scores are reported in units of diameter change per 1\,mm of maximum nodal amplitude,
\begin{equation}
\widehat s^{(s)}_k=\frac{|\mathbf b_k^{(s)\top}\boldsymbol\psi_k^{(s)}|}{\max_a \|\boldsymbol\psi_{k,a}^{(s)}\|_2},
\end{equation}
...
Continuous displacement fields were sampled using positive four-point tetrahedral quadrature, which integrates squared piecewise-linear ($P_1$) displacement norms exactly.
...
To recover the global ring-level character, we therefore evaluate the overall shape change across the entire pelvic continuum.
...
Rather than conditioning on an artificial 1D beam axis, the deformation is decomposed across the entire 3D ring into four canonical deformation families spanning the complete nonrigid polynomial displacement space up to quadratic order:
\begin{enumerate}[label=(\roman*)]
...
\item \textbf{Shear} (9~DOF): uniform transverse shears ($( \tilde y, \tilde x, 0 )$, $( 0, \tilde z, \tilde y )$, $( \tilde z, 0, \tilde x )$) and quadratic transverse shear gradients;
\item \textbf{Torsion} (2~DOF): Saint-Venant twist gradients with zero normal strain,
$\mathbf t_x = (0, -\tilde x \tilde z, \tilde x \tilde y)$ and $\mathbf t_y = (\tilde y \tilde z, 0, -\tilde x \tilde y)$.
\end{enumerate}
...
The combined 23-dimensional nonrigid basis spans the deformation space with low condition number ($\kappa \approx 13.9$ on the reference pelvis).
Because the four deformation families overlap on irregular geometry, the explained nonrigid squared displacement norm was partitioned using Shapley value allocation across all $2^4=16$ subspace combinations:
\begin{equation}
...
Comprehensive mathematical derivations of the continuum inner product, four-point tetrahedral quadrature, canonical polynomial dictionary, cooperative game-theoretic Shapley allocation, numerical conditioning, synthetic backward-recovery benchmarks, and complete 15-mode quantitative profiles are detailed in Appendix~E (Tables~E1--E2).
...
Multiple hypothesis testing across spectral ranks and loading cases was controlled using the Benjamini--Hochberg false discovery rate (FDR) procedure with a family-wise significance threshold of $q < 0.05$; adjusted $p$-values are reported where indicated.
...
\end{equation}
However, the physiological elastic moduli of articular cartilage and symphyseal fibrocartilage ($E_{\rm cartilage} \sim 10\text{--}50\,\mathrm{MPa}$) are approximately two to three orders of magnitude lower than those of cortical and trabecular bone ($E_{\rm bone} \sim 5\text{--}18\,\mathrm{GPa}$).
...
\subsection{Continuum inner product and four-point tetrahedral quadrature}
...
Within any tetrahedron $K$, a $P_1$ displacement field $\mathbf u(\mathbf x)$ is an affine vector function of position:
\begin{equation}
\mathbf u|_K(\mathbf x) = \mathbf A_K \mathbf x + \mathbf b_K, \qquad \mathbf A_K \in \mathbb{R}^{3\times 3}, \quad \mathbf b_K \in \mathbb{R}^3.
\end{equation}
The squared displacement norm integrand $\|\mathbf u(\mathbf x)\|^2 = \mathbf u(\mathbf x) \cdot \mathbf u(\mathbf x)$ is therefore an exact polynomial of degree at most 2 in each coordinate:
\begin{equation}
\|\mathbf u|_K(\mathbf x)\|^2 = \mathbf x^\top \mathbf A_K^\top \mathbf A_K \mathbf x + 2 \mathbf b_K^\top \mathbf A_K \mathbf x + \|\mathbf b_K\|^2 \in \mathcal{P}_2(K).
\end{equation}
To evaluate the continuum volume integral with zero quadrature error, we employ the symmetric four-point Keast/Hammer-Marlowe-Stroud quadrature rule on each tetrahedron $K$.
For a tetrahedron with vertices $\{\mathbf v_1, \mathbf v_2, \mathbf v_3, \mathbf v_4\}$ and volume $|K| = \frac{1}{6}|\det[\mathbf v_2-\mathbf v_1, \mathbf v_3-\mathbf v_1, \mathbf v_4-\mathbf v_1]|$, the four quadrature points $\mathbf x_{K,q}$ ($q=1,2,3,4$) have barycentric coordinates given by the cyclic permutations of:
\begin{equation}
a = \frac{5 + 3\sqrt{5}}{20} \approx 0.5854102, \qquad b = \frac{5 - \sqrt{5}}{20} \approx 0.1381966,
\end{equation}
with uniform quadrature weights $w_{K,q} = \frac{1}{4} |K|$.
Because this four-point rule has degree of precision 2, it integrates any quadratic polynomial over $K$ identically.
Summing across all $N_e$ tetrahedra gives $N_q = 4 N_e$ discrete sampling points with weights $w_p > 0$.
...
The vector space of polynomial displacement fields $\mathbf u(\tilde{\mathbf x})$ of degree at most 2 in three dimensions has dimension $3 \times \binom{3+2}{2} = 30$.
Subtracting the 6 degrees of freedom of infinitesimal rigid motion leaves a 24-dimensional nonrigid polynomial space.
We decompose this space into four canonical physical deformation families spanning 23 nonrigid modes (the remaining single degree of freedom is an isotropic volumetric expansion divergence mode orthogonal to standard modal patterns):
\begin{enumerate}[label=(\roman*)]
...
\item \textbf{Shear family ($d_{\rm shear} = 9$ degrees of freedom):}
Comprises three uniform transverse shears and six quadratic transverse shear gradients:
\begin{equation}
\begin{aligned}
...
\end{equation}
The concatenated design matrix $\mathbf A = [\mathbf Q_{\rm axial}, \mathbf Q_{\rm bending}, \mathbf Q_{\rm shear}, \mathbf Q_{\rm torsion}] \in \mathbb{R}^{3N_q \times 23}$ spans the 23-dimensional canonical nonrigid deformation subspace.

\subsection{Numerical conditioning and the Gram matrix}
On an idealized, centrally symmetric spatial domain (such as a cube or cylinder), the four canonical subspaces are mutually orthogonal with respect to the volume inner product:
\begin{equation}
\mathbf Q_a^\top \mathbf Q_b = \mathbf 0 \qquad \forall a \ne b \in \{\text{axial, bending, shear, torsion}\}.
\end{equation}
In this symmetric limit, the Gram matrix $\mathbf G = \mathbf A^\top \mathbf A = \mathbf I_{23}$ is the identity matrix, with 2-norm condition number $\kappa(\mathbf G) = 1.000$.

On the real, geometrically asymmetric pelvic ring, slight geometric coupling arises, predominantly between bending and shear (both of which involve quadratic cross-terms in $\tilde x_i \tilde x_j$).
The Gram matrix $\mathbf G = \mathbf A^\top \mathbf A \in \mathbb{R}^{23 \times 23}$ for the reference pelvic mesh has condition number
\begin{equation}
\kappa(\mathbf G) = \frac{\sigma_{\max}(\mathbf G)}{\sigma_{\min}(\mathbf G)} \approx 13.887.
\end{equation}
A condition number of $\sim 14$ confirms that the 23 canonical modes remain well conditioned and linearly independent without near-collinearity or numerical rank deficiency.

\subsection{Cooperative game-theoretic Shapley value norm allocation}
Because the four deformation subspaces are not strictly orthogonal on the irregular pelvic anatomy, projecting onto them sequentially (e.g., Gram-Schmidt orthogonalisation) would introduce an arbitrary, order-dependent bias: the first chosen family would absorb all shared variance, artificially starving subsequent families.
...
For any coalition $S$, we construct the concatenated orthonormal basis $\mathbf A_S \in \mathbb{R}^{3N_q \times \sum_{m \in S} d_m}$ and define the characteristic function $v(S)$ as the fraction of nonrigid squared displacement norm explained by the joint subspace:
\begin{equation}
...
\end{equation}
Due to the slight geometric coupling between families ($\kappa \approx 13.9$), the sum $\sum_{m \in M} R^2_m$ may moderately exceed the total explained fraction $R^2(M)$; the Shapley values partition this shared overlap fairly and symmetrically.
...
Its nonrigid shape change is predominantly out-of-plane torsional twist ($23.0\%$) and transverse shear ($17.0\%$), with a total fitted fraction of $42.2\%$.
For Mode~2, nonrigid shape change ($5.5\%$ of total norm) is a coupled sagittal ring flexure and shear ($23.6\%$ bending, $23.4\%$ shear, $20.8\%$ axial, $68.5\%$ fitted).
...
Modes~4 and~5 exhibit strong bending dominance, explaining $43.7\%$ and $47.8\%$ of their nonrigid norm respectively (with standalone bending fits $R^2_{\rm bending}$ reaching $47.6\%$ and $62.4\%$, and total explained fractions of $74.4\%$ and $86.3\%$).
...
In contrast, its near-degenerate partner Mode~10 ($74.6\%$ fitted) is dominated by torsion ($36.4\%$) and shear ($32.4\%$), with minimal axial contribution ($4.8\%$).
