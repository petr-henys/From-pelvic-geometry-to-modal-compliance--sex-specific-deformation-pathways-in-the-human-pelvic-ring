...
Instead, this anatomical opening capacity remains remarkably stable across recurrent deformation subspaces whose internal mode order can swap between individuals.
Comparing skeletons strictly mode-by-mode can therefore create the false impression of functional disruption when the underlying anatomical capacity is substantially more stable at the subspace level.
...
Their close spacing already suggests that inlet-related behaviour may not be consistently attached to one fixed rank label across subjects, even if it remains remarkably stable within the two-dimensional span they share.
...
The 9--10 block is externally separated from both neighbouring modes, whereas the 12--15 regime is left-separated but right-truncated by the eigensolve; in both cases, the dynamics are best interpreted as compact clustered subspaces rather than as a global loss of spectral organisation.
...
Second, basis-invariant anatomical capacities for anatomical diameter functionals (AP and ML inlet; biischiadic and bituberous outlet) provide functionally interpretable coordinates inside each clustered block (Methods).

The rank~9--10 subspace shows the strongest inlet-related capacity among the prevalent clusters.
Its median AP-inlet capacity is \BlockNineTenAP\,mm of inlet diameter change per 1\,mm of maximum nodal displacement, making 9--10 the clearest higher-rank inlet-related program in the cohort.
In contrast, the rank~5--6 cluster shows the strongest transverse outlet-related capacity, with median biischiadic widening \BlockFiveSixBIS\,mm/mm and bituberous response \BlockFiveSixBIT\,mm/mm (accompanied by \BlockFiveSixOUTLETAP\,mm/mm for outlet AP dilation; in the 9--10 subspace, biischiadic widening reaches \BlockNineTenBIS\,mm/mm and outlet AP reaches \BlockNineTenOUTLETAP\,mm/mm).
...
Its anatomical capacity structure is more diffuse than in 9--10 or 5--6, consistent with it representing a broader, higher-order reserve rather than a single sharply interpretable program.
...
\subsection{Subspace-level capacity is substantially more stable than fixed labels in clustered regimes}
\label{sec:label-results}

A direct consequence of this framing follows: in a clustered regime, functional anatomical capacity is substantially more stable at the scale of the subspace than coupling attached to individual spectral ranks.
...
By contrast, the \emph{subspace-level} AP capacity of the 9--10 block remains substantial across both groups (basis-invariant single-functional capacity $\kappa_{{\rm AP},\infty}$: median \ScalarAPNo\ vs.\ \ScalarAPYes\,mm/mm; difference \ScalarAPDifference\,mm/mm, 95\% CI [\ScalarAPLow, \ScalarAPHigh], ratio \ScalarAPRatio, $p = \ScalarAPPformatted$, $r_{\rm rb} = \ScalarAPRankBiserial$; companion joint-SVD score: median \PairAPNo\ vs.\ \PairAPYes\,mm/mm, difference \PairAPDifference\,mm/mm, 95\% CI [\PairAPLow, \PairAPHigh], $p = \PairAPPformatted$).
In sharp contrast to the massive, statistically decisive redistribution seen at the single-mode level ($p < 10^{-21}$), the subspace-level capacity exhibits only modest descriptive variation between non-swapped and swapped groups (difference \ScalarAPDifference\,mm/mm, 95\% CI [\ScalarAPLow, \ScalarAPHigh], $p = \ScalarAPPformatted$).
Across the entire unselected cohort ($N{=}\CohortN$; \CohortNoSwapN\ no-exchange vs.\ \CohortSwapN\ exchange), rank~9 coupling shifts by \CohortRankNineDifference\,mm/mm ($p = \CohortRankNinePformatted$) and rank~10 shifts by $+$\CohortRankTenDifference\,mm/mm ($p = \CohortRankTenPformatted$), whereas subspace-level single-functional capacity differs by only \CohortScalarAPDifference\,mm/mm (the median difference is minimal with a 95\% bootstrap CI of [\CohortScalarAPLow, \CohortScalarAPHigh] spanning zero, and the Mann--Whitney test detects no significant shift across the full cohort, $p = \CohortScalarAPPformatted$, in sharp contrast to the massive single-mode redistribution where $\Delta \approx 0.30$--$0.32$\,mm/mm and $p < 10^{-33}$).
...
Primary cohort routing is evaluated on the total static response; an incremental sensitivity check isolating $\delta\mathbf u_{\rm ext} = K_s^{-1}\mathbf f_{\rm ext}$ on $N=10$ representative subjects confirmed that removing baseline ligament pretension leaves modal dominance rankings and functional conclusions invariant (shifts $\le 3.5\%$; Section~\ref{sec:loading}).
...
\subsection*{Subspace stability resolves the tension between anatomical variability and functional robustness}
...
Habitual load transfer is concentrated in a low-rank backbone, whereas higher-order anatomical deformations remain remarkably stable within compact recurrent subspaces.
The essential biological insight is not merely that spectral labels can become unstable, but that anatomical variation redistributes deformation within a compact local repertoire while maintaining the anatomical capability itself.
...
Our material uncertainty analysis employs first-order Taylor series propagation rather than full non-linear Monte Carlo sampling, which is computationally prohibitive across 278 high-resolution 3D finite-element meshes; while this local linear approximation indicates localized sensitivity trends and covariance-aware gap variances, it represents a first-order approximation that does not capture higher-order non-linear parameter interactions or full non-linear distributions.
Nonetheless, our sensitivity analyses (Appendix~D) indicate that realistic material uncertainty raises overall spectral variability without reorganising the underlying gap architecture or dismantling the clustered regimes.
...
Primary cohort-wide modal strain-energy fractions (Fig.~\ref{fig:fractions-panel}, Table~\ref{tab:dominant-sets}) are computed directly from these total displacement fields $\mathbf u$.
...
To verify that baseline pretension does not confound modal load routing, an incremental sensitivity analysis subtracting $\mathbf u_{\rm pret}$ was performed on a representative subset of $N=10$ subjects (5 exchange, 5 non-exchange across all 5 loads): top recruited modes matched in 100\% of proxy cases and 90\% of standing cases, functional block energy shifts were minor ($\le 3.5\%$; Modes~9--10 inlet energy increased from $71.7\%$ to $75.2\%$), and dominant set sizes ($N_{80}$) were identical, confirming that the primary total-response routing faithfully captures external load recruitment without requiring full-cohort re-simulation.
...
When $\varepsilon_{\rm deg}<\mathrm{gap}_{\rm in}(k)<\varepsilon_{\rm in}$ the eigenvalues are close but distinct; the eigenvectors are formally unique, yet small perturbations can rotate the internal basis substantially while $\mathrm{span}(Q_s)$ itself remains remarkably stable.
...
For $r=2$, the zero-homogeneity of the objective allows parameterization of $\boldsymbol\alpha$ by polar angle $\theta \in [0, \pi)$ with $\boldsymbol\alpha(\theta) = (\cos\theta, \sin\theta)^\top$, solved via high-resolution angular search and scalar refinement; for $r \ge 3$, Nelder--Mead simplex optimisation was used to estimate the optimum; multi-start validation on representative subjects agreed to $<10^{-6}$.
...
Because the four deformation families exhibit intrinsic non-orthogonality (correlated explanatory subspaces) on irregular geometries, Shapley value allocation is employed as an objective game-theoretic method to partition the shared explained norm fairly and symmetrically across the subspaces, rather than acting as a separator of mutually orthogonal physical mechanisms:
\begin{equation}
...
Material uncertainty quantifies how much absolute eigenvalue identifiability would degrade under broader soft-tissue and density-law scatter while showing that the relative fragile-versus-robust gap architecture remains robust.
...
\subsection{Numerical conditioning, family non-orthogonality, and the Gram matrix}
...
Consequently, the four deformation families share correlated explanatory subspaces by mathematical necessity, independently of geometric irregularity.
