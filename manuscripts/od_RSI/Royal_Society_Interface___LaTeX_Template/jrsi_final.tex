...
Our working hypothesis was that sex differences in pelvic mechanics are not reducible to overall size or to any single canal dimension. Instead, scale should set some of the baseline stiffness spectrum, whereas sex should be expressed in selected pathways and in the way load cases distribute response energy across them. We therefore tested three linked expectations: allometry explains a share of the variation in modal stiffness; selected sex differences persist after adjustment for size and age; and habitual support loads and birth-canal-relevant opening probes recruit different parts of the modal repertoire. Support for these expectations would shift the functional interpretation of pelvic dimorphism from static geometry towards mode-specific compliance within a constrained mechanical ring.
...
The analysis used BoneDat, a standardised database of 278 clinical lumbopelvic computed tomography (CT) scans from a Central European cohort spanning adolescence to old age \cite{HenysKuchar2025}. The present dataset comprised 150 female and 128 male individuals aged 16--91 years. Subject selection, segmentation, shape normalisation, template construction, mesh generation and technical validation are reported in the source data descriptor \cite{HenysKuchar2025}; here we summarise only the elements needed for the mechanical analysis. Access to and further use of the database were approved by the Ethics Committee of University Hospital Hradec Kr\'{a}lov\'{e} (Ref: 202411IO3P), as reported in the source data descriptor \cite{HenysKuchar2025}.
...
The principal size variable was the cube root of total pelvic volume relative to the template reference volume, $S=(V/V_{\mathrm{template}})^{1/3}$. This gives a whole-pelvis linear scale for a three-dimensional structure and avoids using a single canal diameter as a proxy for body size. We also recorded total surface area $A$, total mass $m$, effective thickness $t_{\mathrm{eff}}=V/A$ and effective density $\rho_{\mathrm{eff}}=m/V$, where $V$ denotes total pelvic volume. These effective quantities are bulk descriptors used for cohort-level comparison rather than local measurements of cortical thickness or calibrated tissue density.
...
Two terms are used throughout the manuscript. Pathway accessibility denotes the position of a mode in the stiffness spectrum. Pathway dominance denotes how much of a particular load response is carried by that mode after loading is applied.


\subsection{Mechanical probes and modal participation}

Four static loading scenarios were analysed. Bilateral and unilateral stances represented habitual support mechanics, whereas inlet opening and outlet opening served as probes with obstetric relevance, but not as literal simulations of childbirth. The probe configurations used anatomically defined regions on the reference template to prescribe comparable support constraints and force directions. The same region definitions and loading logic were then applied to the subject-specific finite-element models in the custom DOLFINx workflow, so that each probe response was solved on the corresponding individual pelvis while retaining a standardised anatomical definition of the load case.


For each subject, the resulting displacement field was represented on the same correspondence-preserving domain as the subject-specific modes and was then projected onto that subject's modal basis. Thus, variation in modal participation reflects both the subject-specific probe response and the subject-specific spectrum, while the anatomical definition of each probe remains standardised. With $\Phi=[\phi_{1},\ldots,\phi_{10}]$, the participation vector $q$ was obtained from
\begin{equation}
...
We next asked which soft-tissue constraints most affected the modal spectrum. This was the template-level component of the workflow. For computational tractability, local perturbation operators for ligament stiffness and interface modulus were prepared once on the population template and evaluated in the common template-referenced coordinate system. For each selected mode, we calculated first-order sensitivities of the eigenvalue to small local changes in ligament stiffness or interface modulus. These local, linearised measures rank how strongly a local stiffening would change the elastic cost of an $M$-normalised deformation pathway.
...
The sensitivity analysis covered the symphyseal ligament group, anterior SIJ ligaments, posterior SIJ ligaments, interosseous SIJ ligaments, sacrospinous ligaments and sacrotuberous ligaments, together with the sacroiliac cartilage interfaces and pubic symphysis. These template-defined sensitivities were used to rank the anatomical constraints associated with selected modes, rather than to model subject-specific tissue perturbations. The primary focus was on modes showing the clearest adjusted sex effects.

\subsection{Statistical analysis}

Sex differences in cohort descriptors were assessed with Welch two-sample $t$-tests. Empirical allometry of surface area, mass, effective thickness and effective density was evaluated with log--log regressions against pelvic scale.
...
Because scale was defined from total pelvic volume, the volume--scale relation is definitional and is not reported as an empirical allometry. Among the remaining quantities, total surface area scaled sub-isometrically with pelvic scale (slope $=1.684$, $R^2=0.894$), total mass increased with scale (slope $=2.679$, $R^2=0.902$), effective thickness increased with scale (slope $=1.316$, $R^2=0.837$), and effective density declined modestly with scale (slope $=-0.321$, $R^2=0.117$; table~\ref{tab:allometry}).
...
This is a more precise interpretation than saying that female pelves are simply more compliant. The difference lies in the organisation of compliance: support mechanics remain broadly shared, while opening probes redistribute the response in a sex-specific way. Outlet opening supports a less evident, but similar view because mode~5 dominated in both sexes, yet female pelves retained greater concurrent mode~9 participation. Inlet and outlet mechanics therefore appear to be overlapping expressions of one constrained ring rather than independent modules.
...
Several limitations define the scope of this interpretation. The cohort represents a Central European clinical CT sample and was not linked to obstetric outcomes. The finite-element formulation captures the small-deformation elastic regime, uses phantomless CT-derived material descriptors and omits subject-specific nonlinear tissue behaviour. The mechanical probes used standardised template-derived anatomical definitions for supports and loading directions, which improves comparability but does not capture all individual variation in physiological loading. Conversely, the local sensitivity analysis used template-defined perturbation operators and should be interpreted as a ranking of constraint influence rather than as a full subject-specific perturbation study.
...
This study used retrospectively acquired, anonymised clinical CT data released through the BoneDat resource. According to the BoneDat dataset documentation, the study was approved by the Ethics Committee of University Hospital Hradec Kr\'{a}lov\'{e} (Ref: 202411IO3P). The requirement for individual informed consent was waived by the ethics committee because the study used retrospectively acquired anonymised imaging data.

\section*{Use of Artificial Intelligence (AI) and AI-assisted technologies}
During manuscript preparation, AI-assisted tools were used for language editing, structural checks and preparation of submission-readiness recommendations. All scientific content, analyses, interpretations and final text were reviewed and approved by the authors, who take full responsibility for the manuscript.

\section*{Data accessibility}

The BoneDat source morphology dataset is available through the published data descriptor \cite{HenysKuchar2025}. Processed derived datasets supporting the present analyses, together with the analysis scripts, simulation code, model-parameter files, figure-generation scripts and data dictionary, are deposited in Figshare \cite{HenysPelvicModalFigshare2026}. The shared files do not include the original clinical CT scans.
...
Effective thickness (mm) & $4.96 \pm 0.36$ & $5.51 \pm 0.36$ & $4.44 \times 10^{-29}$ \\
Effective density (kg\,m$^{-3}$) & $1341 \pm 79$ & $1321 \pm 74$ & 0.028 \\
...
Effective thickness & 1.316 & $1.25 \times 10^{-110}$ & 0.837 \\
...
\begin{table}[p]
\centering
\caption{Mode-wise scale dependence and adjusted sex effects. Correlations are Pearson's $r$ between $\log(S)$ and $\log(\lambda)$ before and after size normalisation, $\lambda_{\mathrm{scaled}}=\lambda_{\mathrm{raw}}(S/S_0)^{\gamma_k}$. Sex effects are male-versus-female percentage differences from $\log(\lambda_{\mathrm{raw}})\sim\log(S)+\mathrm{sex}+\mathrm{age}$; $q_{\mathrm{sex}}$ values are Benjamini--Hochberg adjusted across modes.}
...
\caption{Soft-tissue sensitivity profiles for selected modes. Values are cohort means from template-defined local perturbation operators, with bilateral structures averaged. Percentages are within-class contributions for each mode, calculated separately for interface and ligament sensitivities because these measures have different definitions.}
...
\caption{Fractional modal decomposition across standardised subject-specific loading probes. Each small pie chart shows the cohort mean contribution of modes~1--10 to the modal response. Segment colours denote individual modal components (modes~1--10), and labels within selected sectors indicate dominant modes.}
...
\caption{Representative modal deformation patterns are shown for each standardised loading probe. Insets indicate the corresponding load application and the most strongly recruited mode, while wireframe overlays illustrate the dominant deformation pathway.}
\label{fig:workflow}
\end{figure}
