...
The mechanical model was intentionally simple and comparative. It used small-deformation, linear isotropic elasticity. Rigid support was imposed by penalty constraints, and the major pelvic ligament groups were represented as linear axial springs attached to corresponding anatomical regions. The model was therefore used to compare relative elastic pathway accessibility across individuals. It was not intended to reproduce nonlinear ligament recruitment, pregnancy-related remodelling, tissue failure or the full mechanics of labour.
...
The analyses use modes~1--10. Per-subject permutation vectors stored in the eigenvalue workbooks were used to keep mode labels aligned across individuals, so a reported mode number refers to the corresponding low-order deformation family rather than simply to the unprocessed solver order. Because eigenvectors are sign-indeterminate, arbitrary sign changes do not affect the scalar quantities reported here: eigenvalues, modal energy fractions or sensitivity measures.
...
Four static loading scenarios were analysed. Bilateral and unilateral stances represented habitual support mechanics. Inlet opening and outlet opening were treated as pelvic-opening probes with obstetric relevance, but not as literal simulations of childbirth. The additional LAB phase-3 output retained in the original working files was not included in the present analysis.

For each loading scenario, the finite-element solution produced a displacement field $u$. We did not interpret this field only in physical space. Instead, we projected it onto the modal basis to determine which deformation pathways carried the response. With $\Phi=[\phi_{1},\ldots,\phi_{10}]$, the participation vector $q$ was obtained from
\begin{equation}
...
We next asked which soft-tissue constraints most affected the modal spectrum. For each selected mode, we calculated first-order sensitivities of the eigenvalue to small local changes in ligament stiffness or interface modulus. These sensitivities rank how strongly a local stiffening would change the elastic cost of an $M$-normalised deformation pathway. They are local, linearised measures; they are not simulations of ligament cutting, pregnancy-related remodelling or nonlinear failure.
...
\end{equation}
The sex coefficient was expressed as the percentage change in eigenvalue for male relative to female pelves after adjustment for pelvic scale and age. Because the ten modes form an ordered mechanical spectrum rather than ten unrelated endpoints, we considered effect size, direction and anatomical coherence, and also report Benjamini--Hochberg false-discovery-rate adjusted $q$-values.

To test how much conventional pelvimetry added beyond scale and age, we fitted female-only models for modes~9 and~2:
\begin{equation}
\log(\lambda_{\mathrm{raw}}) \sim \log(\mathrm{scale}) + \mathrm{age} + \mathrm{dimension}.
\end{equation}
For each dimension, we report the incremental explanatory contribution as partial $R^2$.

Modal participation fractions were summarised descriptively as cohort means because they are compositional quantities. To quantify whether a load response was concentrated in one mode or spread across several modes, we also calculated the effective number of participating modes,
\begin{equation}
N_{\mathrm{eff}} = \exp\left(-\sum_i p_i\log p_i\right),
\end{equation}
where $p_i$ is the energy fraction of mode $i$ after normalisation over modes~1--10. Larger values indicate a less concentrated modal response. Group comparisons of $N_{\mathrm{eff}}$ are reported as supporting descriptive statistics.
...
Before descriptive size scaling, log pelvic scale was negatively correlated with log eigenvalue for all ten modes (table~\ref{tab:modes} and figure~\ref{fig:eigs}). After descriptive size scaling, the correlation was near zero for modes~1 and~5 and positive for several other modes. The strongest positive scaled correlations were observed for modes~2 and~9.
...
The load projections clarify how these pathways are used. Bilateral and unilateral stance recruited a common low-order support pattern in both sexes, whereas inlet opening exposed a different organisation of response energy. Mode~9 was the largest inlet-opening component in almost all subjects, but male pelves concentrated more of the response into this one mode. Female pelves showed a higher effective number of participating modes, indicating that the inlet-opening response was spread across a broader set of coordinated pathways.
...
The present study does not imply two fundamentally different pelvic systems in female and male individuals, and it does not demonstrate childbirth outcome or labour performance directly. Its claim is narrower: both sexes share a support-oriented low-order repertoire, but differ in the accessibility and recruitment of selected deformation pathways, especially the inlet-opening pathway represented by mode~9. In this framework, the relevant phenotype is not pelvic size, canal shape or laxity alone, but the organisation of mode-specific compliance within a constrained ring.
