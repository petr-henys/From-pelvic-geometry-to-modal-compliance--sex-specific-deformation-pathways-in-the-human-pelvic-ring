...
That framing remains useful, but it is incomplete. The pelvis is not merely a set of distances. It is a mechanically coupled ring whose behaviour depends on the interaction of geometry, material distribution, sacroiliac and symphyseal interfaces, and ligamentous constraints. A pelvis does not function in locomotion or childbirth as a static assemblage of diameters. It functions as a deformable structure whose performance depends on which deformation pathways are available, how strongly they are recruited by a given load, and which anatomical constraints regulate access to them.

Several strands of work make this broader view necessary. Experimental studies indicate that wider female pelves do not inevitably incur a locomotor penalty \cite{Warrener2015}. Recent syntheses of the obstetrical dilemma have shown that childbirth should neither be dismissed as a myth nor reduced to a one-dimensional opposition between locomotion and canal size \cite{Pavlicev2020,Haeusler2021,Grunstra2023}. More broadly, pelvic form must satisfy several partially competing demands at once, including load transfer, childbirth, pelvic-floor support and developmental robustness \cite{Pavlicev2020,Haeusler2021,Grunstra2019,Grunstra2023}. Population studies have shown substantial within- and between-population variation in the shape of the birth canal \cite{BettiManica2018,Auerbach2018}, while ontogenetic work indicates that adult pelvic form is historically and developmentally contingent rather than determined by one selective pressure alone \cite{VerbruggenNowlan2017,Novak2020,KurkiDecrausaz2015}. Taken together, these observations imply that static pelvimetry captures only part of pelvic function.

What remains limited is a population-scale framework that describes pelvic mechanics directly. Subject-specific finite-element studies have already shown that pubic symphysis stiffness, sacroiliac constraints and load direction materially affect pelvic behaviour \cite{Eichenseer2011,Hammer2013}. This is biomechanically plausible, because the sacroiliac joint is a specialised load-transfer articulation with small but functionally meaningful motion, and because its ligamentous architecture is central to force closure and ring stability \cite{Vleeming2012,Kiapour2019,ChoKwak2020}. Yet isolated subject-specific models do not readily reveal which features of mechanical behaviour are shared across individuals, which vary with scale, and which are organised by sex.

Here we address that gap using correspondence-preserving modal analysis of 278 anatomically standardised human pelves. Modal decomposition treats the pelvis not as a rigid set of obstetric diameters but as a repertoire of deformation pathways with different stiffness costs. This shift separates two distinct questions. First, which pathways are mechanically accessible? Second, once a pathway is available, how is response organised under a given load? That distinction is central to childbirth-related mechanics. A pelvis may permit access to a pathway without concentrating its response into that pathway, and a mechanically important sex difference may lie in response organisation rather than in gross widening or global softening.

Our central proposition is that the female pelvis is best understood not as an evolutionarily compromised structure and not as a generally loosened ring, but as a balanced compliant ring. In this framework, pelvic size should primarily modulate the accessibility of deformation pathways, whereas sex should primarily modulate how the ring organises response once those pathways are engaged. We therefore test four linked propositions. First, allometry should explain a substantial share of stiffness variation, but not all of it. Second, sex differences should persist in selected modes after adjustment for scale and age, indicating genuine reorganisation of ring mechanics rather than trivial allometric scaling. Third, locomotor load cases should recruit a shared low-order support backbone, whereas childbirth-motivated load cases should reveal sex-biased redistribution of modal energy. Fourth, the principal inlet-opening pathway should be governed chiefly by the pubic symphysis and sacroiliac constraints rather than by static canal dimensions alone. If supported, these propositions would imply that pelvic function is better described as an architecture of dynamic compliance than as a simple geometric trade-off.
...
Allometric regressions showed near-isometric scaling of total volume with pelvic scale (slope $=3.000$, $R^2=1.000$), shallower scaling of total surface area (slope $=1.684$, $R^2=0.894$), increasing total mass with scale (slope $=2.679$, $R^2=0.902$), and a modest decline in effective density with scale (slope $=-0.321$, $R^2=0.117$; table~\ref{tab:allometry}). Pelvic size therefore explains an important part of baseline stiffness variation and, by implication, much of the accessibility of deformation pathways.

This point requires careful interpretation. In the modal sense relevant to the present eigenproblem, smaller geometrically similar structures are expected to exhibit higher unscaled eigenvalues, and larger ones lower eigenvalues. Because female pelves were smaller on average, a purely size-driven effect would predict systematically higher eigenvalues in women across modes. That expectation was met for most of the unscaled spectrum, but not all of it.

\subsection{Size correction reveals selective, not diffuse, sex differences}

Two modes already departed from the expected raw size trend: women showed lower unscaled eigenvalues than men in modes~2 and~9 despite smaller average pelvic size. These reversals were especially informative, because they indicated that sex-specific mechanical organisation was already strong enough to overcome the baseline stiffening expected for a smaller ring.
...
This pattern argues against the idea of a globally lax female pelvis. A general softening of the ring would be expected to shift the spectrum broadly in one direction. Instead, sex effects were selective and concentrated in a restricted subset of pathways.

\subsection{Static pelvimetry captures only a modest fraction of mechanical phenotype}

In female-only models, PIT explained 13.5\% of the variation in size-scaled mode~9 eigenvalues and AP explained 9.2\%. For mode~2, the strongest single predictor was PIT, with $R^2=0.089$, while all other dimensions explained less than 8\% of the variance (table~\ref{tab:staticassoc}). No single linear measure captured more than a modest share of the relevant modal variation.

Static pelvimetry therefore remains informative, but only weakly so. Mechanical phenotype is distributed across geometry, material distribution and constraint architecture, and cannot be recovered from any single obstetric diameter.

\subsection{Habitual support mechanics rely on a shared low-order backbone}

Under bilateral stance, the response in both sexes was dominated by mode~2 (84.6\% in women and 84.5\% in men). Unilateral stance broadened recruitment but preserved the same low-order support architecture: modes~1--3 accounted for most of the response in both sexes. Terminal descent likewise returned to a mode~2-dominated response (77.2\% in women and 74.7\% in men), with mode~8 as the main secondary contributor.
...
\subsection{Childbirth-related loading reveals sex-specific organisation of response}
...
This point is central. Women did not differ from men primarily in whether mode~9 existed. They differed in how inlet-opening response was organised once that pathway became available. Men concentrated more of the response into a single dominant route. Women spread that response more broadly across a coordinated low-order repertoire.
...
\subsection{Mode~9 is governed by an anteriorly weighted ring constraint system}
...
Mode~9 is therefore distinctive not only because it is sex-biased and inlet-opening dominant, but also because it is regulated chiefly by an anteriorly weighted sacroiliac--symphyseal constraint system. The principal inlet-opening pathway is thus a property of the whole pelvic ring rather than of gross inlet width alone.

\section{Discussion}

The central contribution of this study is not that it reiterates familiar static dimorphism in pelvic dimensions. Those differences are real and are clearly present in our data, but they are not the main discovery. The main result is that pelvic mechanics remain sexually structured even after scale correction, and that this structure is functionally interpretable. The female pelvis is therefore not simply a smaller pelvis, not simply a wider pelvis, and not a generally softened pelvis. It is a differently organised ring.

That conclusion changes the level at which pelvic function should be discussed. Instead of treating function as a direct readout of canal dimensions, it becomes more productive to treat the pelvis as a compliant mechanical system with a structured repertoire of accessible deformations. This is why the concept of the balanced compliant ring is useful. It does not deny morphology. It places morphology within a broader mechanical architecture.

The first necessary step toward this conclusion was allometric separation. Size is a major driver of pelvic stiffness, and any discussion of female compliance that ignores scale risks triviality. In the modal sense relevant here, smaller similar structures are expected to exhibit higher raw eigenvalues, and larger ones lower raw eigenvalues. The fact that modes~2 and~9 already reversed that expected trend in the unscaled data is therefore highly informative. Those pathways are not just statistically different after correction; they are sufficiently reorganised that sex-specific structure is already visible against the background of allometry.

The corrected spectrum then shows that the remaining sex differences are selective rather than diffuse. This matters because it argues strongly against diffuse female laxity. If the female pelvis were simply globally more compliant, the spectrum would be expected to shift more uniformly. Instead, support-related mechanics remain concentrated in the same low-order backbone in both sexes. Bilateral stance, unilateral stance and terminal descent all rely on essentially the same support architecture. In other words, the female pelvis does not abandon routine load-transfer competence in order to gain childbirth-relevant mobility.

Mode~9 is the clearest case because it uniquely links three otherwise separate observations. First, it belongs to the subset of pathways that cannot be explained by allometry alone. Second, it retains a strong sex difference after size correction. Third, it becomes the dominant pathway under simulated inlet opening in the great majority of subjects. That combination makes mode~9 more than a statistically significant eigenmode. It is the point at which the sex-specific modal phenotype becomes mechanically legible.

Its mechanistic signature sharpens that interpretation. Mode~9 is only modestly predicted by static inlet dimensions. Instead, it is regulated most strongly by the pubic symphysis, anterior sacroiliac ligaments and sacroiliac cartilage interfaces. This localisation is biomechanically important because it makes the principal inlet-opening pathway a property of the whole ring rather than a simple property of inlet diameter. It also explains why static pelvimetry captures only a modest fraction of the relevant mechanical variation.

The accessibility--dominance distinction is equally important. It would be easy to overstate the finding by claiming that women are simply more compliant in mode~9. That is not what the results show. Mode~9 is more accessible in women after size correction, but women do not place a larger fraction of inlet-opening response into mode~9 itself. Instead, they distribute the response more broadly across a coordinated set of low-order pathways. Men, by contrast, concentrate more of the inlet-opening response into mode~9. This is why the female phenotype is best described as distributed compliance rather than as focal or diffuse laxity.

That distribution matters biologically. It suggests that childbirth-relevant deformation in the female pelvis is not achieved by dissolving ring integrity but by preserving everyday support while selectively redistributing response under childbirth-motivated loading. In that sense, locomotor stability and childbirth-related compliance do not sit on a single linear axis. They can be partitioned across partly distinct features of ring mechanics.

This interpretation fits well with broader reassessments of the obstetrical dilemma. Recent syntheses have argued that childbirth cannot be reduced to a simple opposition between locomotion and canal size \cite{Pavlicev2020,Haeusler2021,Grunstra2023}, and experimental work indicates that wider pelves need not impose a substantial locomotor penalty \cite{Warrener2015}. Our results are consistent with that view, but add a concrete mechanical formulation: the apparent conflict between support and childbirth may be mitigated because the pelvis can preserve a shared support backbone while selectively accessing childbirth-relevant deformation pathways.

The study nevertheless has clear limitations. The models are linear and small-deformation, and the cohort represents Central European morphology rather than global human diversity. The analyses do not incorporate pregnancy-specific soft-tissue changes, muscle forces, fetal interaction or labour outcomes. The omitted \texttt{pretension\_sensitivity.xlsx} workbook also means that our sensitivity interpretation is limited to stiffness and interface effects. These limitations define the next step rather than weakening the present conclusion. The key advance here is the establishment of a coherent population-level framework in which pelvic mechanics can be described through modal phenotype.

Future work should test whether the same pathway structure is preserved in pregnancy-aware nonlinear models, whether it predicts clinically relevant obstetric behaviour, and whether similar mechanical organisation is observed across more diverse human populations. A particularly important extension will be to link modal phenotype to outcome-linked datasets rather than to loading proxies alone.
