...
\address{$^{1}$Institute of New Technologies and Applied Informatics, Faculty of Mechatronics, Informatics and Interdisciplinary Studies, Technical University of Liberec, Liberec, Czech Republic\\$^{2}$Department of Anatomy, Faculty of Medicine in Hradec Kr\'{a}lov\'{e}, Charles University, Hradec Kr\'{a}lov\'{e}, Czech Republic\\$^{3}$Division of Macroscopic and Clinical Anatomy, Gottfried Schatz Research Center, Medical University of Graz, Graz, Austria\\$^{4}$Division of Macroscopic and Clinical Anatomy, Gottfried Schatz Research Center, Medical University of Graz, Graz, Austria; Department of Orthopedic and Trauma Surgery, University of Leipzig, Leipzig, Germany; Division of Biomechatronics, Fraunhofer IWU, Dresden, Germany}
...
Several lines of work make this broader view necessary. Wider female pelves do not inevitably impose a locomotor penalty \cite{war2015}. Recent evolutionary and obstetric syntheses have argued that childbirth should not be reduced to a one-dimensional opposition between locomotion and canal size \cite{gru2023,pav2020}. Human pelvic form also shows substantial geographic and developmental variation \cite{bet2018,ver2017,nov2020}, which makes it unlikely that any single linear dimension can stand in for pelvic function in general.

What remains limited is a population-scale mechanical framework. Subject-specific finite-element studies have shown that sacroiliac constraints, pubic symphysis stiffness and load direction materially affect pelvic behaviour \cite{eic2011,ham2013}. This is biomechanically plausible because the sacroiliac joint is a specialised load-transfer articulation with limited but functionally meaningful motion \cite{vle2012}. Yet isolated case studies do not readily show which features of behaviour are shared across individuals, which vary with scale, and which remain sexually structured after size is taken into account.
...
To identify which structures regulate the most relevant pathways, we analysed workbook-based sensitivities of eigenvalues to changes in ligament stiffness and interface modulus using ligament\_sensitivity.xlsx and cartilage\_sensitivity.xlsx. The main mechanistic focus was mode 9, because it combined strong sex structure with dominant inlet-opening recruitment. Modes 2 and 5 were retained as support-related and outlet-related reference modes. Pretension perturbations were reviewed separately but were not integrated into the main hierarchy because they probe prestress rather than constitutive stiffness of the base eigenproblem.

\subsection{Statistical analysis}
Static sex differences were assessed with Welch two-sample t-tests. Mode-wise sex effects were estimated using linear models of the form log($\lambda$) ~ log(scale) + sex + age, with the sex coefficient expressed as the percentage difference for males relative to females after adjustment for scale and age. Female-only univariate models related size-scaled eigenvalues of modes 9 and 2 to individual pelvic dimensions in order to test how much of the mechanical phenotype could be recovered from conventional pelvimetry.

\section{Results}
\subsection{Cohort structure and static dimorphism}
The cohort reproduced the expected static sex pattern (table 1). Female pelves were smaller in overall scale but showed larger AP and PIT measures, a wider subpubic angle, and larger biischiadic and bituberous widths. Sacral width did not differ significantly between the sexes.

\subsection{Allometry sets the baseline accessibility landscape}
Allometric regression showed near-isometric scaling of total volume with pelvic scale (slope 3.000, R$^2$ = 1.000), shallower scaling of total surface area (1.684, R$^2$ = 0.894), increasing total mass with scale (2.679, R$^2$ = 0.902), and a modest decline in effective density (−0.321, R$^2$ = 0.117; table 2). In practical terms, this means that raw eigenvalues cannot be interpreted without reference to size. In the present modal framework, smaller geometrically similar structures are expected to show higher raw eigenvalues. That expectation was met across much of the spectrum, but not completely: modes 2 and 9 already departed from it in the unscaled data, with female means lower than male means despite smaller average female pelvic size.

\subsection{Scale-corrected eigenstructure reveals selective sex effects}
After adjustment for scale and age, the strongest sex effects were retained in modes 2 and 9, where male pelves were 20.7\% and 20.1\% stiffer, respectively (figure 1; table 3). Substantial positive male--female differences were also present in modes 4 (15.9\%) and 8 (14.5\%), with smaller but significant effects in modes 3, 7 and 10. By contrast, mode 1 was effectively neutral, and modes 5 and 6 did not retain robust sex effects after adjustment. The residual sex signal was therefore concentrated in selected pathways rather than diffused across the full spectrum.

\subsection{Support and childbirth-motivated loads recruit a shared repertoire with sex-biased weighting}
Under bilateral stance, mode 2 dominated the response in every individual and accounted for about 85\% of mean modal participation in both sexes (table 4; figure 2). Unilateral stance broadened recruitment but remained built on the same low-order repertoire, dominated by modes 1 and 2 with a smaller contribution from mode 3. These scenarios therefore point to a largely shared support backbone.
...
In female-only models, the strongest single predictor of size-scaled mode 9 stiffness was PIT (R$^2$ = 0.135), followed by AP (R$^2$ = 0.092), iliopectineal eminence width (R$^2$ = 0.086) and sacral width (R$^2$ = 0.077; table 5). For mode 2, PIT again provided the strongest single association (R$^2$ = 0.089), followed by bituberous width (R$^2$ = 0.078). No single static dimension explained more than a modest proportion of the variance in either pathway.

\subsection{Mode 9 localises inlet-opening mechanics to anterior ring constraints}
Sensitivity analysis identified mode 9 as the most clearly localised childbirth-relevant pathway (figure 3; table 6). The largest interface sensitivities were found in the left and right sacroiliac cartilage interfaces and the pubic symphysis interface. At the ligament level, the strongest effect was associated with the symphyseal ligament group, followed by the anterior sacroiliac ligaments and then the interosseous ligaments. Posterior sacroiliac, sacrospinous and sacrotuberous ligaments were markedly less influential. Relative to reference modes 2 and 5, mode 9 therefore showed a distinctly anteriorly weighted sensitivity profile.

\section{Discussion}
The main contribution of this study is not the rediscovery of familiar pelvic dimorphism. Static sex differences in pelvic size and obstetric dimensions are real, but they are not the most informative result here. The more important finding is that pelvic mechanics remain selectively sexually structured even after size is taken into account, and that this structure can be interpreted in terms of ring mechanics rather than only pelvic diameters.

Allometric separation is essential to that interpretation. In the present modal framework, raw eigenvalues are influenced both by pelvic organisation and by overall size. In simple biomechanical terms, a smaller pelvis with otherwise similar geometry is expected to exhibit higher raw eigenvalues than a larger one. Any discussion of female compliance that ignores this point risks collapsing into a scaling artefact. Correcting for scale and age is therefore not a technical afterthought; it is what makes mechanical interpretation possible. The persistence of sex effects after that correction indicates that the female and male pelvis differ in how stiffness is distributed across the ring, not merely in absolute size.

At the same time, the results need to be read at two distinct mechanical levels. The eigenvalue spectrum addresses pathway accessibility: it tells us which deformation patterns are mechanically easier or harder to recruit. The loading scenarios address pathway recruitment: they show how a particular load is decomposed across the available modes. Keeping those levels separate clarifies several points that are easy to blur if modal fractions are treated as though they measured compliance directly.

This distinction is especially useful for interpreting the support scenarios. Bilateral and unilateral stance recruited the same low-order backbone in both sexes, which argues against any simple picture of the female pelvis as globally lax. Yet the shared backbone is not mechanically identical. Mode 2, the dominant bilateral support pathway, remained sexually differentiated after size correction. That does not contradict the idea of a shared backbone; it refines it. The low-order support architecture is shared, but it is not indifferent to sex. It appears to be tuned differently within the same ring system.

Mode 9 is central for a different reason. It should not be highlighted because women rely on it more strongly during inlet opening; in fact, the fractional inlet response was less concentrated in mode 9 in women than in men. Mode 9 matters because it sits at the intersection of three otherwise separate observations. First, it already runs against the raw allometric expectation. Second, it retains one of the strongest sex effects after adjustment for scale and age. Third, it remains the dominant inlet-opening component in the great majority of individuals. Taken together, those features make mode 9 the clearest mechanically interpretable inlet-opening pathway in the present framework.

The lower female portion of mode 9 under inlet loading therefore strengthens, rather than weakens, the central interpretation. It shows that female childbirth-relevant mechanics are not achieved by leaning more heavily on a single mode. Instead, the female pelvis appears to distribute inlet-opening response across a broader low-order set while retaining mode 9 as the principal axis of that response. In other words, female inlet opening looks less like focal reliance on one pathway and more like coordinated use of several nearby pathways within a shared repertoire. This is why the female pelvis is better described as balanced than as merely compliant.

The sensitivity analysis sharpens that point. Mode 9 was governed chiefly by the pubic symphysis, the anterior sacroiliac ligaments and the sacroiliac cartilage interfaces, not by diffuse relaxation of the ring. That localisation makes the inlet-opening pathway a property of the whole pelvic ring, especially its anterior constraint architecture, rather than a simple readout of inlet width. It also helps explain why static pelvimetry explained only a modest fraction of the variance in the mechanically relevant modes. Canal dimensions matter, but they do not exhaust pelvic mechanics.

Taken together, these results support a more precise reformulation of the classical locomotion--childbirth tension. The data do not suggest two completely different pelvic systems in women and men. Nor do they suggest that childbirth-relevant compliance is purchased by abandoning ordinary load-transfer competence. Instead, the same ring retains a shared support backbone while tuning the accessibility and weighting of selected low-order pathways. This interpretation is consistent with broader reassessments of the obstetrical dilemma, which argue that childbirth cannot be reduced to canal size alone and that locomotor function is not necessarily compromised by a wider or differently organised female pelvis \cite{gru2023,war2015,pav2020,bet2018}. The present study adds a population-scale mechanical formulation to that broader debate.

Several limitations should be made explicit. The models are linear and restricted to small-deformation mechanics. The cohort represents retrospective clinical CT data from a Central European population rather than an obstetric outcomes cohort, and the anatomy is non-pregnant. Material mapping relies on phantom-less calibration and should be interpreted relatively rather than as direct densitometry. The present work also uses standardised loading probes rather than patient-specific labour trajectories. Finally, ligament pretension was explored separately but not integrated into the main hierarchy, because the principal sensitivity analysis was defined around constitutive stiffness and interface perturbations of the base eigenproblem. These limitations do not weaken the core result; they define the next step.

Future work should test whether the same pathway structure persists in pregnancy-aware and non-linear models, whether it predicts clinically relevant obstetric behaviour, and whether similar tuning is observed across more diverse human populations. The most important extension will be to connect modal phenotype to outcome-linked data rather than to loading proxies alone.

\section{Conclusion}
This study shows that the human pelvis is best interpreted as a load-dependent compliant ring. Allometry explains much of the baseline accessibility landscape, but it does not remove the sex signal. After adjustment for scale and age, the strongest residual differences were concentrated in selected low-order modes, especially modes 2 and 9. Habitual support loads recruited a shared backbone in both sexes, whereas childbirth-motivated loading revealed sex-biased weighting within a shared low-order repertoire. Mode 9 emerged as the clearest inlet-opening pathway and was regulated primarily by the pubic symphysis, anterior sacroiliac ligaments and sacroiliac cartilage interfaces. The female pelvis is therefore not best understood as globally lax, but as a balanced compliant ring that preserves everyday support while selectively tuning childbirth-relevant deformation.

\section*{Ethics}
The analysis used retrospectively acquired clinical CT data released within the BoneDat resource \cite{hen2025}. The final ethics wording should be checked against the dataset paper and local approvals before submission.

\section*{Data accessibility}
The source morphology dataset is available through BoneDat \cite{hen2025}. Derived tables, figure-generation code and manuscript source files should be archived in a public repository before submission; the repository identifier can be inserted at final revision.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\linewidth]{manuscript_assets/fig1_sex_effects.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\linewidth]{manuscript_assets/fig2_recruitment.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\linewidth]{manuscript_assets/fig3_sensitivity.png}
...
Measure & Female $(n=150)$ & Male $(n=128)$ & $p$ \\\midrule
...
\begin{tabular}{lccc}\toprule
Outcome versus scale & Slope & $p$ & $R^2$ \\\midrule
...
Mode & $r$ (unscaled) & $p$ & $r$ (scaled) & $p$ & Sex effect \\\midrule
...
Scenario & Sex & M1 & M2 & M3 & M4 & M5 & M6 & M7 & M8 & M9 & M10 \\\midrule
...
& \multicolumn{2}{c}{Mode 9} & \multicolumn{2}{c}{Mode 2} \\\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Dimension & $R^2$ & $p$ & $R^2$ & $p$ \\\midrule
...
\begin{tabular}{lccc}\toprule
Structure & Mode 9 sensitivity & Mode 5 sensitivity & Mode 2 sensitivity \\\midrule
...
\bibitem{ros2002} Rosenberg K, Trevathan WR. 2002 Birth, obstetrics and human evolution. \emph{BJOG} \textbf{109}, 1199--1206.
\bibitem{tre2015} Trevathan WR. 2015 Primate pelvic anatomy and implications for birth. \emph{Philos. Trans. R. Soc. B} \textbf{370}, 20140065.
\bibitem{gru2023} Grunstra NDS \emph{et al.} 2023 There is an obstetrical dilemma: misconceptions about the evolution of human childbirth and pelvic form. \emph{Am. J. Biol. Anthropol.} \textbf{181}, 535--544.
\bibitem{war2015} Warrener AG, Lewton KL, Pontzer H, Lieberman DE. 2015 A wider pelvis does not increase locomotor cost in humans, with implications for the evolution of childbirth. \emph{PLoS ONE} \textbf{10}, e0118903.
\bibitem{pav2020} Pavli\v{c}ev M, Romero R, Mitteroecker P. 2020 Evolution of the human pelvis and obstructed labor: new explanations of an old obstetrical dilemma. \emph{Am. J. Obstet. Gynecol.} \textbf{222}, 3--16.
\bibitem{bet2018} Betti L, Manica A. 2018 Human variation in the shape of the birth canal is significant and geographically structured. \emph{Proc. R. Soc. B} \textbf{285}, 20181807.
\bibitem{ver2017} Verbruggen SW, Nowlan NC. 2017 Ontogeny of the human pelvis. \emph{Anat. Rec.} \textbf{300}, 643--652.
\bibitem{nov2020} Nov\'{a}k J, Br\r{u}\v{z}ek J, Zamrazilov\'{a} H, Va\v{n}kov\'{a} M, Hill M, Sedl\'{a}k P. 2020 The relationship between adolescent obesity and pelvis dimensions in adulthood: a retrospective longitudinal study. \emph{PeerJ} \textbf{8}, e8951.
\bibitem{eic2011} Eichenseer PH, Sybert DR, Cotton JR. 2011 A finite element analysis of sacroiliac joint ligaments in response to different loading conditions. \emph{Spine} \textbf{36}, E1446--E1452.
\bibitem{ham2013} Hammer N, Steinke H, Lingslebe U, Bechmann I, Josten C, Slowik V, B\"ohme J. 2013 Ligamentous influence in pelvic load distribution. \emph{Spine J.} \textbf{13}, 1321--1330.
\bibitem{vle2012} Vleeming A, Schuenke MD, Masi AT, Carreiro JE, Danneels F, Willard FH. 2012 The sacroiliac joint: an overview of its anatomy, function and potential clinical implications. \emph{J. Anat.} \textbf{221}, 537--567.
\bibitem{hen2025} Heny\v{s} P, Kucha\v{r} M. 2025 BoneDat, a database of standardised bone morphology for in silico analyses. \emph{Sci. Data} \textbf{12}, 1043.
\end{thebibliography}
\end{document}