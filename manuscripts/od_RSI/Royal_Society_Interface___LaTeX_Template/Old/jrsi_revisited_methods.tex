...
\begin{abstract}
The human pelvis must simultaneously support habitual bipedal locomotion and accommodate the rare but extreme mechanical demands of childbirth. Classical interpretations frame this duality as a problem of static geometry and evaluate it through pelvimetric dimensions, implicitly treating the pelvis as a rigid structure. Here we propose an alternative framework in which pelvic function is defined by load-dependent access to deformation pathways within a compliant ring system. We analysed 278 anatomically standardised lumbopelvic finite-element models derived from clinical CT using correspondence-preserving modal decomposition, allometric normalisation, standardised locomotor and obstetric load cases, and sensitivity analysis of ligamentous and cartilaginous constraints. Allometry explained a substantial proportion of global stiffness, but size-normalised analyses revealed persistent sex differences in specific modes, most prominently in modes~2 and~9. Under bilateral and unilateral stance, both sexes recruited a shared low-order support backbone. Under simulated inlet opening, however, male pelves concentrated a larger fraction of the response in a single dominant mode, whereas female pelves distributed deformation more broadly across coordinated low-order pathways. Sensitivity analysis showed that the principal inlet-opening pathway is regulated primarily by the pubic symphysis, anterior sacroiliac ligament complexes and sacroiliac cartilage interfaces, rather than by static bony dimensions alone. We therefore interpret the female pelvis not as simply wider or globally more compliant, but as a balanced compliant ring in which locomotor stability is preserved while childbirth-relevant deformation pathways remain selectively accessible. This reframes pelvic function from a static geometric trade-off to a problem of mechanical architecture and dynamic compliance.
\end{abstract}

\begin{fmtext}
This manuscript is a conceptually revised, submission-oriented version of the study draft, reorganised around a single mechanistic argument: the female pelvis is best interpreted as a balanced compliant ring that preserves shared locomotor support while providing selectively accessible, anteriorly regulated pathways for childbirth-relevant deformation.
\end{fmtext}
...
Our analysis is organised around a single central idea. We propose that the female pelvis is best understood not as a globally loosened structure, nor as a simple geometric compromise, but as a \emph{balanced compliant ring}. In this view, habitual locomotor support remains built upon a largely shared low-order backbone in both sexes, while childbirth-relevant compliance is achieved through selective access to particular deformation pathways. Those pathways, in turn, are expected to be regulated not by gross widening alone but by an anterior constraint architecture involving the pubic symphysis and the sacroiliac interfaces.

We therefore test four linked propositions. First, pelvic size should explain a substantial fraction of stiffness variation, but not all of it. Second, some sex differences should remain after adjustment for scale and age, indicating a genuine reorganisation of ring mechanics rather than a trivial allometric effect. Third, locomotor load cases should recruit a shared low-order modal support basis, whereas obstetric load cases should reveal sex-biased redistribution within that basis. Fourth, the principal inlet-opening pathway should be governed primarily by anterior constraints rather than by static dimensions alone. If supported, these propositions would imply that pelvic function is best described not by geometry in isolation but by the architecture of dynamic compliance.

\section{Material and methods}

\subsection{Cohort and correspondence-preserving anatomical framework}

The analysis used the BoneDat resource, a standardised database of 278 clinical lumbopelvic CT scans from a Central European cohort spanning adolescence to old age \cite{HenysKuchar2025}. In the present derived workbooks, the cohort comprised 150 female and 128 male individuals, with ages ranging from 16 to 91 years. BoneDat is especially valuable because it provides not only segmented anatomy, but also a shape-normalisation pipeline that maps each subject into a common reference space while preserving subject-specific morphology through deformation fields. In practical terms, this means that inlet and outlet geometry, density-derived stiffness, ligament attachment regions and load-case responses can all be represented on the same reference domain and compared directly across individuals.

The original clinical scans were acquired on a Siemens Definition AS+ scanner and were retained only if voxel resolution was sufficiently fine for cohort-scale mechanical analysis. BoneDat further provides curated multi-label segmentations of the pelvis, sacrum and lower lumbar vertebrae, a rigid--affine--deformable registration workflow, a common reference surface, and volumetric meshes suitable for finite-element conversion. For the present analysis, each released deformation field was used to warp a high-quality tetrahedral template mesh into subject space, after which all mechanics were evaluated on the fixed reference domain by pullback. This correspondence-preserving design is critical for our interpretation because it allows mode-wise statistics and sensitivity summaries to be compared vertex-by-vertex and region-by-region across the entire cohort.

Mesh quality and registration fidelity were screened against the technical validation criteria reported for BoneDat. Cases that failed basic quality control were excluded upstream in the released resource. Because the original CT data were acquired without a calibration phantom, absolute bone mineral density cannot be recovered directly. Instead, phantom-less internal calibration using the released soft-tissue HU references was applied, and calibrated density values were converted into hydroxyapatite-equivalent density before assignment of elastic properties. The resulting density field should therefore be interpreted as a cohort-wise relative material descriptor rather than as direct clinical densitometry.

\subsection{Allometry, static morphology and cohort descriptors}

Allometric descriptors were taken from \texttt{allometry.xlsx}. Let $B=\{\mathrm{LI},\mathrm{RI},\mathrm{S}\}$ denote the pelvic subregions represented on the shared mesh topology. For each subject, the split and enclosed pelvic volumes and surface areas were computed from native volumetric and surface operators, yielding total volume, total surface area and their region-specific components. Additional global descriptors included total mass, effective thickness and effective density. Effective thickness was defined as the ratio of total volume to total area and provides a compact summary of mean bony shell thickness independent of local curvature. Effective density was defined as the ratio of total mass to total volume and estimates average tissue density across the pelvic complex.

The principal scale variable was defined as the cube root of total pelvic volume relative to a template reference volume. Because linear dimensions scale with the cube root of volume, this index provides an appropriate measure of whole-pelvis size while remaining robust to density shifts. To isolate density-driven from geometry-driven effects, template-based masses were also recomputed on fixed reference coordinates using the mapped density field. This separation allowed global size, geometry-driven variation and density-derived variation to be contrasted rather than conflated.

Static morphological dimensions were extracted from \texttt{anatomy\_data.xlsx}. The main variables were the anteroposterior inlet diameter (AP), pelvic inlet transverse diameter (PIT), subpubic angle, biacetabular width, biischiadic width, bituberous width, sacral width and iliopectineal eminence width. These descriptors were retained not because we expected them to fully recover pelvic mechanics, but because they represent the conventional language in which sexual dimorphism and obstetric adequacy are most often discussed.

\subsection{Finite-element formulation}

We analysed the mechanical eigenstructure of the human pelvis and its static response under physiologically motivated load cases. The computational workflow comprised five linked steps: conversion of the released anatomy into a common finite-element domain; region identification and material assignment; optional assembly of ligament spring bundles; solution of a generalised elastic eigenproblem; and projection of static load responses onto the modal basis.

Let $\Omega \subset \mathbb{R}^{3}$ denote the reference finite-element domain. Individual morphology was represented by a displacement parametrisation $\varphi:\Omega \rightarrow \mathbb{R}^{3}$ sampled at the finite-element degrees of freedom, so that the deformation gradient was
\begin{equation}
F = I + \nabla \varphi, \qquad J = \det F, \qquad F^{-1} = (I + \nabla \varphi)^{-1}.
\end{equation}
The reference template corresponds to $\varphi=0$. Boundary surfaces from the original FEBio model were mapped to exterior facets and tagged with integer markers, producing a face-wise measure of anatomical loading regions on the common domain.

We used a first-order vector Lagrange space for displacements and a discontinuous cell-wise constant space for material parameters. In the main model, bone, sacroiliac cartilage and pubic symphysis were represented on the common tetrahedral mesh. Bone material properties were spatially heterogeneous, whereas the cartilage and symphyseal regions were assigned region-wise uniform properties.

\subsection{Constitutive model and material assignment}

Linear isotropic elasticity was used throughout, with Lam\'e parameters
\begin{equation}
\mu = \frac{E}{2(1+\nu)}, \qquad
\lambda = \frac{E\nu}{(1+\nu)(1-2\nu)}.
\end{equation}
The mapped small-strain tensor and Cauchy stress were
\begin{equation}
\varepsilon(w)=\mathrm{sym}(\nabla w\,F^{-1}), \qquad
\sigma(w)=\lambda\,\mathrm{tr}(\varepsilon(w))I+2\mu\varepsilon(w).
\end{equation}

Bone Young's modulus was derived from hydroxyapatite density at template surface points and interpolated to cell degrees of freedom using radial basis function smoothing. Before interpolation, the code applied the piecewise empirical law
\begin{equation}
\rho_{\mathrm{ash}}=\frac{\rho_{\mathrm{HA}}+0.09}{1.14}, \qquad
E(\rho_{\mathrm{ash}})=
\begin{cases}
2398, & \rho_{\mathrm{ash}}\leq 0.486,\\
10200\,\rho_{\mathrm{ash}}^{2}, & \rho_{\mathrm{ash}}>0.486.
\end{cases}
\end{equation}
Bone used a uniform Poisson ratio. Identified sacroiliac cartilage regions and the pubic symphysis received uniform $(E,\nu)$ values by region and could be included explicitly in the sensitivity analyses.

All integrals were assembled on the reference domain using the mapping induced by $\varphi$. The symmetric stiffness bilinear form was
\begin{equation}
a(u,v)=\int_{\Omega} J\,\sigma(u):\varepsilon(v)\,\mathrm{d}x,
\end{equation}
and the consistent mass form was
\begin{equation}
m(u,v)=\int_{\Omega} J\,(u,v)\,\mathrm{d}x.
\end{equation}
An auxiliary reference mass form,
\begin{equation}
m_{0}(u,v)=\int_{\Omega}(u,v)\,\mathrm{d}x,
\end{equation}
was also assembled to stabilise mode pairing across samples.

Rigid fixation on tagged support surfaces was enforced using a symmetric penalty augmentation of the stiffness bilinear form,
\begin{equation}
a(u,v)\leftarrow a(u,v)+\gamma\int_{\Gamma_{\mathrm{fix}}}J_{s}(u,v)\,\mathrm{d}s,
\qquad
J_{s}=\|J\,F^{-T}n\|,
\end{equation}
where $n$ is the unit normal to the reference facet and $J_{s}$ is the surface Jacobian of the pullback.

\subsection{Ligament spring bundles}

If enabled, discrete sets of polyline bundles were converted into axial springs between the nearest finite-element degrees of freedom. For a ligament segment connecting two blocks of degrees of freedom $i$ and $j$ with current endpoints $x_i$ and $x_j$, length $L=\|x_j-x_i\|$ and unit direction $e=(x_j-x_i)/L$, the per-segment material and geometric contributions were
\begin{equation}
W=\frac{EA_{\ell}}{n_{\mathrm{seg}}L}\,ee^{T}, \qquad
W_{g}=\frac{T/n_{\mathrm{seg}}}{L}\,(I-ee^{T}) \quad \text{if } T>0,
\end{equation}
where $EA_{\ell}$ is the total axial stiffness of the bundle, $n_{\mathrm{seg}}$ is the number of segments in that bundle, and $T$ is the total pretension assigned to the bundle. These contributions were assembled into the global stiffness matrix with the standard block pattern. Pretension also contributed static nodal forces at the segment endpoints. In the present manuscript, the ligament system is important chiefly because it allows us to test which bundles most strongly regulate childbirth-relevant modes.

\subsection{Generalised eigenproblem}

For each subject, the elastic eigenvalue problem sought mode shapes $\{\phi_{k}\}$ and eigenvalues $\{\lambda_{k}\}$ such that
\begin{equation}
K(\varphi,E,\nu)\phi_{k} = \lambda_{k} M(\varphi)\phi_{k}, \qquad k=1,2,\ldots
\end{equation}
where $K$ and $M$ are the assembled stiffness and consistent mass matrices on the mapped domain. Rigid-body modes were detected and removed using a small-eigenvalue threshold. In the interpretation used throughout this paper, lower eigenvalues correspond to mechanically more accessible deformation pathways.

The workbooks provided both unscaled and size-corrected eigenvalue series (\texttt{eigen\_data.xlsx} and \texttt{eigen\_data\_scaled.xlsx}). Following the original workflow, we focused on modes~1--10 as the mechanically relevant spectrum for the present biological interpretation.

\subsection{Static loads and pullbacks}

Static load responses solved
\begin{equation}
Ku=b
\end{equation}
with loads defined on tagged surfaces. The implementation supported both pressure-based and traction-based specification with geometric pullback to the reference domain.

For a scalar pressure intensity $p$ prescribed on a physical surface, the right-hand side term was
\begin{equation}
L_{p}(v)=-\int_{\Gamma} p\,n_{\Sigma}\cdot v\,\mathrm{d}\Sigma
       =-\int_{\Gamma_{\mathrm{ref}}} p\,(J F^{-T} n)\cdot v\,\mathrm{d}S.
\end{equation}
For a traction intensity $t$, the pullback was
\begin{equation}
L_{t}(v)=\int_{\Gamma} t\cdot v\,\mathrm{d}\Sigma
       =\int_{\Gamma_{\mathrm{ref}}}(J_{s}t)\cdot v\,\mathrm{d}S.
\end{equation}
If a desired resultant force rather than a pointwise intensity was prescribed, a uniform pressure or traction was selected so that the integrated force matched the target.

Five standardised loading scenarios were analysed. Two represented habitual support mechanics: bilateral stance (\texttt{SP2leg}) and unilateral stance (\texttt{SP1leg}). Three represented childbirth-motivated conditions: inlet opening (\texttt{LAB\_phase1}), outlet opening (\texttt{LAB\_phase2}) and terminal descent (\texttt{LAB\_phase3}). These were not treated as literal reproductions of labour, but as mechanically controlled probes that distinguish support-related from childbirth-related deformation pathways.

\subsection{Dataset-specific morphology and material updates}

Each sample in the dataset provided a template-point displacement field and a bone-density field. Morphology and material properties were therefore updated on the common domain by interpolation,
\begin{equation}
\varphi(X)=\mathcal{I}[u^{\mathrm{pts}}](X), \qquad
E(X)=\mathcal{I}[E(\rho_{\mathrm{HA}})](X),
\end{equation}
for degree-of-freedom coordinates $X$. The solvers were then reassembled with the new $(E,\nu)$ field and $\varphi$, allowing both the stiffness matrix and the mass matrix to vary across subjects while remaining represented on the same reference topology.

\subsection{Modal analysis of static responses}

Let $\Phi=[\phi_{1},\ldots,\phi_{m}]$ denote the matrix of elastic eigenvectors and let $u$ be a static displacement field generated by one of the load cases. Without assuming that the modes are $M$-orthonormal, the modal participation factors $q$ were obtained by solving the Gram system
\begin{equation}
(\Phi^{T}M\Phi)q=\Phi^{T}Mu.
\end{equation}
This formulation is equivalent to a least-squares modal projection under the mass metric and ensures that participation factors remain comparable even if numerical normalisation differs between subjects.

Mode norms were defined by
\begin{equation}
\|\phi_{i}\|_{M}^{2}=\phi_{i}^{T}M\phi_{i},
\end{equation}
and modal strain energies and fractions were computed as
\begin{equation}
U_{i}=\frac{1}{2}\lambda_{i}q_{i}^{2}\|\phi_{i}\|_{M}^{2}, \qquad
\eta_{i}=\frac{U_{i}}{\sum_{j}U_{j}}.
\end{equation}
For interpretation and table construction, we normalised fractions over modes~1--10 because the supplied variable documentation identified the first ten modes as the last relevant group for biological interpretation. This choice also keeps the load-case summaries focused on the mechanically meaningful low-order spectrum.

\subsection{Sensitivity analysis}

To identify which soft-tissue and interface properties regulate the most important deformation pathways, we analysed workbook-based sensitivities of eigenvalues to ligament stiffness and cartilage-related modulus changes. For a ligament bundle with axial stiffness $EA_{\ell}$, the first-order sensitivity of mode $i$ was
\begin{equation}
\frac{\partial \lambda_{i}}{\partial EA_{\ell}}
=
\frac{\phi_{i}^{T}\left(\partial K/\partial EA_{\ell}\right)\phi_{i}}
{\phi_{i}^{T}M\phi_{i}}.
\end{equation}
This quantity directly expresses how strongly the stiffness of a given mode would change under an infinitesimal perturbation of that ligament bundle.

For cartilage and symphyseal regions, the code reported a normalised energy-based sensitivity proxy. For a region $R$ with modulus $E_{R}$, the per-mode elastic energy over that region was
\begin{equation}
\mathcal{E}_{i}(R)=\int_{R} J\,\sigma(\phi_{i}):\varepsilon(\phi_{i})\,\mathrm{d}x,
\end{equation}
and the reported sensitivity proxy was
\begin{equation}
S_{i,R}=\frac{\mathcal{E}_{i}(R)}{E_{R}\,\phi_{i}^{T}M\phi_{i}}.
\end{equation}
Under linearisation, this quantity is proportional to $\partial \lambda_{i}/\partial E_{R}$ and therefore provides a directly interpretable way to rank the importance of cartilage regions and the pubic symphysis for a given mode.

In the present paper, our primary focus was mode~9 because the results identify it as the principal candidate inlet-opening pathway. For reference, we also considered sensitivity values for modes~2 and~5, which behaved as support-related and outlet-related modes, respectively.
...
\end{equation}
The corresponding sex coefficient was interpreted as the percent change in modal stiffness for males relative to females after adjusting for scale and age.

To evaluate whether static pelvimetry can recover mechanically relevant differences, we performed female-only univariate models relating size-corrected eigenvalues of modes~9 and~2 to single pelvic dimensions. This design was chosen because the central obstetric interpretation concerns the structure of female compliance rather than sex classification itself.
...
The picture changed sharply under the inlet-opening scenario. Mode~9 was the dominant response component in 265 of the 278 subjects. However, its contribution differed between sexes in a way that is easy to misread unless energy concentration and mechanical strategy are carefully distinguished.
...
The important point is therefore not that women have a stronger mode~9 in terms of energy concentration. They do not. Rather, women appear to gain access to inlet-opening behaviour through a more distributed low-order strategy. Men reach a similar class of deformation with a greater concentration into one dominant pathway. This distinction underlies our interpretation of the female pelvis as a balanced compliant ring rather than as a generally more lax structure.
...
We therefore argue that the female pelvis is best understood as a balanced compliant ring: a structure that preserves everyday locomotor support while maintaining selectively accessible, anteriorly regulated pathways for childbirth-relevant deformation. This reframes pelvic biomechanics from a question of static geometry to one of mechanical architecture and dynamic compliance.

\section*{Ethics}

This study used retrospectively acquired clinical CT data released through the BoneDat resource. The exact wording of the ethics approval statement and protocol number should be verified against the final dataset documentation before submission.
...
[Insert final funding statement.]

\section*{Acknowledgements}

We thank the BoneDat project for providing the standardised anatomical framework on which this analysis is based. Additional technical and institutional acknowledgements can be added in the final submission version.
