...
\end{equation}
and report the incremental explanatory contribution of each pelvic dimension as partial $R^2$. Size-scaled eigenvalues were retained only for descriptive visualisation of the modal spectrum and for supplementary comparison of residual allometric patterning; they were not used as the dependent variable in models that simultaneously adjusted for pelvic scale.
...
The sensitivity analyses provided important complementary evidence. Although limited to the template-based framework, they identify which components of the model exert the strongest control over the mechanically relevant pathways. The regulatory capacity of pelvic soft tissues is already well recognised; what the present results add is evidence that this regulation is mode-selective and may provide a mechanism for transiently modulating access to specific deformation pathways.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\linewidth]{Figures/Fig2_variantB.png}
\caption{Residualized modal values and adjusted sex effects. Panel~A shows distributions of residualized log eigenvalues by sex across modes~1--10 after adjustment for $\log(\mathrm{scale})$ and age. Panel~B shows adjusted sex effects estimated from models of the form $\log(\lambda_{\mathrm{raw}}) \sim \log(\mathrm{scale}) + \mathrm{sex} + \mathrm{age}$, highlighting modes~2 and~9 as the strongest sex-biased pathways and mode~1 as a near-neutral reference.}
\label{fig:eigs}
\end{figure}
...
\caption{Static pelvimetry captures only part of the mechanically relevant variation. In female-only models adjusted for pelvic scale and age, AP and PIT are the strongest single predictors of mode~9 stiffness, whereas bituberous, biischiadic and biacetabular breadths are more informative for mode~2. Even so, no single linear dimension explains more than a minority of the variance.}
