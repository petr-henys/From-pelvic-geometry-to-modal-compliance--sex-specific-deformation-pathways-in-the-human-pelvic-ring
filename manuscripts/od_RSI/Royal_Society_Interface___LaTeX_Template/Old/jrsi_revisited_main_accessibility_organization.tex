...
We therefore test four linked propositions. First, pelvic size should explain a substantial fraction of stiffness variation, but not all of it, and should primarily affect how mechanically accessible childbirth-relevant pathways are. Second, some sex differences should remain after adjustment for scale and age, indicating a genuine reorganisation of ring mechanics rather than a trivial allometric effect. Third, locomotor load cases should recruit a shared low-order modal support basis, whereas obstetric load cases should reveal sex-biased redistribution within that basis. Fourth, the principal inlet-opening pathway should be governed primarily by anterior constraints rather than by static dimensions alone. If supported, these propositions would imply that pelvic function is best described not by geometry in isolation but by the architecture of dynamic compliance, in which size modulates accessibility and sex modulates organisation of response.

\section{Material and methods}

\subsection{Cohort and correspondence-preserving framework}

The analysis used the BoneDat resource, a standardised database of 278 clinical lumbopelvic CT scans from a Central European cohort spanning adolescence to old age \cite{HenysKuchar2025}. In the present derived workbooks, the cohort comprised 150 female and 128 male individuals, aged 16--91 years. BoneDat provides curated segmentations, a common reference anatomy, and deformation fields that map each subject into a shared template domain while preserving subject-specific morphology. We used these released deformation fields to warp a high-quality tetrahedral template mesh into subject space and evaluated all mechanics on the fixed reference domain by pullback. This correspondence-preserving design allowed geometry, density-derived stiffness, ligament attachment regions and load-case responses to be compared directly across individuals.

Mesh quality and registration fidelity were screened against the technical validation criteria reported for BoneDat. Because the source CT scans were acquired without a calibration phantom, absolute mineral density cannot be interpreted as clinical densitometry. Instead, phantom-less internal calibration was used to convert Hounsfield units into hydroxyapatite-equivalent density before assignment of elastic properties. These density fields were therefore treated as cohort-wise relative material descriptors rather than direct in vivo measurements.

\subsection{Allometry and static morphology}

Allometric descriptors were taken from \texttt{allometry.xlsx}. The principal scale variable was defined as the cube root of total pelvic volume relative to the reference template, providing a whole-pelvis size index suitable for log--log allometric analysis. Additional global descriptors included total surface area, total volume, total mass, effective thickness and effective density. To separate geometry-driven from density-driven variation, template-based mass summaries were also recomputed on fixed reference coordinates using the mapped density field.

Static morphological dimensions were extracted from \texttt{anatomy\_data.xlsx}. The retained variables were the anteroposterior inlet diameter (AP), pelvic inlet transverse diameter (PIT), subpubic angle, biacetabular width, biischiadic width, bituberous width, sacral width and iliopectineal eminence width. These conventional descriptors were included to relate modal mechanics to the language of static pelvimetry.

\subsection{Finite-element model and eigenanalysis}

We analysed the elastic eigenstructure of the human pelvis and its static response under physiologically motivated loading. Let $\Omega \subset \mathbb{R}^{3}$ denote the reference finite-element domain and let $\varphi:\Omega \rightarrow \mathbb{R}^{3}$ denote the displacement parametrisation representing subject-specific morphology on that domain. The deformation gradient was
\begin{equation}
F = I + \nabla \varphi, \qquad J = \det F.
\end{equation}
Bone, sacroiliac cartilage and pubic symphysis were represented on the common tetrahedral mesh. Bone material properties were spatially heterogeneous, whereas cartilage and symphyseal regions were assigned region-wise uniform properties.

Linear isotropic elasticity was used throughout. Bone Young's modulus was derived from hydroxyapatite density and interpolated to cell degrees of freedom; cartilage and symphyseal regions received configurable regional $(E,\nu)$ values. The symmetric stiffness and consistent mass forms were assembled on the mapped reference domain,
\begin{equation}
a(u,v)=\int_{\Omega} J\,\sigma(u):\varepsilon(v)\,\mathrm{d}x,
\qquad
m(u,v)=\int_{\Omega} J\,(u,v)\,\mathrm{d}x.
\end{equation}
If enabled, ligament bundles were represented as axial spring systems assembled between anatomically defined endpoints.

For each subject, we solved the generalised eigenvalue problem
\begin{equation}
K(\varphi,E,\nu)\phi_{k} = \lambda_{k} M(\varphi)\phi_{k}, \qquad k=1,2,\ldots
\end{equation}
after removal of rigid-body modes. Lower eigenvalues were interpreted as mechanically more accessible deformation pathways. Both unscaled and size-corrected eigenvalue series were available from the derived workbooks, and modes~1--10 were treated as the biologically relevant low-order spectrum.

\subsection{Loading scenarios and modal projection}

Static load responses solved
\begin{equation}
Ku=b
\end{equation}
with loads defined on anatomically tagged surfaces and pulled back to the reference domain. We analysed five standardised loading scenarios: bilateral stance (\texttt{SP2leg}), unilateral stance (\texttt{SP1leg}), inlet opening (\texttt{LAB\_phase1}), outlet opening (\texttt{LAB\_phase2}) and terminal descent (\texttt{LAB\_phase3}). These scenarios were not intended as literal simulations of labour, but as mechanically controlled probes distinguishing support-related from childbirth-related deformation pathways.

Let $\Phi=[\phi_{1},\ldots,\phi_{m}]$ denote the matrix of elastic eigenvectors and let $u$ be a static displacement field. Modal participation factors were obtained by solving
\begin{equation}
(\Phi^{T}M\Phi)q=\Phi^{T}Mu,
\end{equation}
which gives the least-squares modal coordinates under the mass metric. Modal strain energies and fractions were then computed as
\begin{equation}
U_{i}=\frac{1}{2}\lambda_{i}q_{i}^{2}\|\phi_{i}\|_{M}^{2}, \qquad
\eta_{i}=\frac{U_{i}}{\sum_{j}U_{j}}.
\end{equation}
For interpretation and table construction, fractions were normalised over modes~1--10, consistent with the supplied variable documentation.
...
{\phi_{i}^{T}M\phi_{i}}.
\end{equation}
For cartilage and symphyseal regions, the code reported a normalised energy-based sensitivity proxy proportional to $\partial \lambda_{i}/\partial E_{R}$ under linearisation. In the present manuscript, mode~9 was the primary focus because it emerged as the principal candidate inlet-opening pathway; modes~2 and~5 were considered as support-related and outlet-related references.
...
\end{equation}
The sex coefficient was interpreted as the percent change in modal stiffness for males relative to females after adjustment for scale and age.

To evaluate whether static pelvimetry can recover mechanically relevant differences, we performed female-only univariate models relating size-corrected eigenvalues of modes~9 and~2 to single pelvic dimensions.
...
The first necessary step toward this conclusion was allometric separation. Size is a major driver of pelvic mechanics, and any attempt to discuss female compliance without accounting for scale risks triviality. In our cohort, however, the size effect did not behave as a simple ``larger means stiffer'' rule. Instead, larger pelves tended to show lower unscaled eigenstiffness in the low-order spectrum, including mode~9, meaning that the inlet-opening pathway became mechanically more accessible with increasing size. Yet the persistence of strong adjusted sex effects in modes~2 and~9 shows that size alone is still insufficient. There is therefore a genuine reorganisation of how stiffness is distributed across deformation pathways once size has been taken into account.

Mode~9 is the clearest case. It is sex-biased but not female-exclusive. It is strongly recruited by simulated inlet opening but not by habitual stance. It is only modestly predicted by static inlet dimensions. It is also highly size-dependent in unscaled form, becoming more accessible in larger pelves, yet it retains a strong sex effect after size correction. Finally, it is governed primarily by the pubic symphysis and anterior sacroiliac structures. That combination of features makes it more than a statistically interesting eigenmode. It makes it the best candidate for an inlet-opening pathway at the level of the whole pelvic ring and clarifies that size and sex do not act on the same component of the phenotype.
...
This is why the load-case results matter so much. Men and women did not differ primarily in whether mode~9 existed, nor does the sex difference reduce to the statement that one sex simply ``has more'' of mode~9 than the other. Rather, they differed in how inlet-opening response was organised around that pathway. Men concentrated a greater fraction of energy into mode~9 itself, whereas women distributed the response more broadly across mode~9, mode~4 and additional low-order components. Because mode~9 is intrinsically softer in female pelves, it is likely to become accessible at lower energetic cost. Under that condition, inlet-opening behaviour need not be concentrated in a single dominant mode, but can instead be expressed through a broader combination of coordinated low-order modes. The key distinction is therefore that size modulates how accessible the pathway is, whereas sex modulates how the pelvic ring uses that pathway once engaged. This is the key empirical basis for the notion of distributed compliance in the female pelvis.

That pattern should not be misinterpreted as diffuse laxity. On the contrary, diffuse laxity would be mechanically undesirable because it would threaten everyday support stability. Our findings point toward something much more selective. Women retain a locomotor backbone that is remarkably similar to that of men under bilateral and unilateral stance. What differs is that childbirth-relevant loading reveals a wider coordinated set of low-order pathways once the inlet-opening regime is entered. In that sense, the female pelvis appears not softer in general, but \emph{differently organised} in how it expresses compliance. Larger size may facilitate access to the relevant pathway, but sex shapes whether the ensuing response remains concentrated or becomes distributed.
...
\subsection{Size modulates accessibility, sex modulates organisation}

One concise way to summarise the mechanical logic of our findings is that pelvic size and sex act at different levels of the phenotype. Increasing pelvic size reduces the unscaled eigenstiffness of mode~9 and related low-order modes, thereby making the inlet-opening pathway more mechanically accessible. Sex, however, acts more strongly on the organisation of the response once that pathway becomes available. Female pelves do not simply show ``more'' mode~9; rather, they tend to express inlet-opening behaviour through broader co-recruitment of coordinated low-order modes, whereas male pelves concentrate more of the same response in mode~9 itself. This separation helps resolve an otherwise confusing pattern in the data: a pathway can be softer and more accessible in one group without necessarily carrying the largest energetic fraction in that same group. Accessibility and energetic concentration are therefore not interchangeable, and distinguishing them is necessary for a coherent interpretation of pelvic compliance.
