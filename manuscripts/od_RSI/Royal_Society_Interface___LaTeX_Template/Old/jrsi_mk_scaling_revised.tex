...
Four static loading scenarios were analysed. Bilateral and unilateral stances represented habitual support mechanics, whereas inlet opening and outlet opening served as pelvic-opening probes with obstetric relevance. The additional LAB phase-3 output retained in the original working files was not included in the present analysis.
...
For the descriptive size-normalised eigenvalue summaries shown in table~\ref{tab:modes} and figure~\ref{fig:eigs}, we used an empirical allometric normalisation. Let $S_i$ denote the volume-derived pelvic scale of subject $i$, and let $S_0=\exp(\overline{\log S})=0.9568452943$ denote the geometric-mean scale of the cohort. For each mode $k$, the scale exponent was estimated from the sex-adjusted log--log model
\begin{equation}
\log(\lambda_{ik,\mathrm{raw}})=a_k+b_k\log(S_i)+c_k\,\mathrm{sex}_i+\varepsilon_i.
\end{equation}
The descriptive size-normalised eigenvalue was then calculated as
\begin{equation}
\lambda_{ik,\mathrm{scaled}}=\lambda_{ik,\mathrm{raw}}\left(\frac{S_i}{S_0}\right)^{\gamma_k},
\qquad \gamma_k=-b_k.
\end{equation}
For modes~1--10, the corresponding $\gamma_k$ values were 1.382, 1.834, 1.676, 2.393, 2.068, 2.139, 2.346, 2.326, 1.887 and 2.136. These scaled values were used only for descriptive distributions and scale-dependence summaries. All inferential models used raw eigenvalues with scale included explicitly as a covariate.
