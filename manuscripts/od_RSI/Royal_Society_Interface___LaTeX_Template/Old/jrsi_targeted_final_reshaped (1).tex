...
Allometric regressions showed near-isometric scaling of total volume with pelvic scale (slope $=3.000$, $R^2=1.000$), shallower scaling of total surface area (slope $=1.684$, $R^2=0.894$), and increasing total mass with scale (slope $=2.679$, $R^2=0.902$), while effective density decreased modestly (slope $=-0.321$, $R^2=0.117$; table~\ref{tab:allometry}). Size therefore explains an important part of baseline stiffness variation and, by implication, the accessibility of deformation pathways. This point has a clear mechanical direction: for geometrically similar elastic structures, smaller size is expected to increase rather than decrease stiffness. Because female pelves are smaller on average in our cohort, a purely size-driven effect would therefore predict systematically higher eigenvalues in women across modes.

That expectation was met for most, but not all, of the unscaled spectrum (table~\ref{tab:modes}). Women showed higher raw eigenvalues in modes~1, 5, 6, 7 and~10, consistent with the expected size effect, but they showed lower raw eigenvalues in modes~2 and~9 despite their smaller average size. These two exceptions are especially informative because they indicate that sex-specific mechanical organisation is already strong enough to overcome the baseline stiffening expected for a smaller pelvic ring.

After adjustment for scale and age, several modes retained clear sex differences (table~\ref{tab:modes}). The largest effects were observed in mode~2 and mode~9, where male pelves were stiffer than female pelves by $20.72\%$ and $20.07\%$, respectively. Significant differences were also present in modes~3, 4, 7, 8 and~10, whereas mode~1 remained effectively neutral. Size correction therefore did not create the sex difference; it clarified a mode-specific reorganisation that was already visible in selected pathways of the raw spectrum.
...
\subsection{Inserted new} In the pooled unscaled spectrum, women generally showed higher eigenvalues than men, consistent with the modal size effect expected for a smaller geometrically similar structure, but this pattern was not uniform: modes~2 and~9 already showed lower eigenvalues in women despite their smaller average pelvic size. These reversals therefore motivated size correction, which separated baseline allometry from sex-specific organisation and confirmed modes~2 and~9 as the clearest pathways in which stiffness is redistributed rather than merely scaled. Importantly, this did not translate into lower functional joint mobility in women: across all standardised scenarios, women exhibited larger SIJ rotations and translations, with SIJ motion relating most clearly to mode~2 and less consistently to mode~9.
...
Inserted new: The allometric point should be read in the modal sense relevant to the present eigenproblem, not as a statement about organism-level load adequacy. In bending-dominated structures, static load--deflection stiffness follows the familiar beam scaling, whereas free-vibration eigenvalues satisfy $K\phi=\lambda M\phi$ with $\lambda=\omega^{2}$; under geometric similarity, this implies higher modal eigenvalues in smaller similar structures and lower unscaled eigenvalues in larger ones. The lower unscaled eigenvalues in males therefore do not imply mechanically poorer everyday support, but the expected modal signature of a larger pelvic ring within a standardised modelling framework. By the same logic, the higher raw eigenvalues seen in women across much of the unscaled spectrum should not be mistaken for reduced functional mobility, because women nonetheless showed larger SIJ rotations and translations across all standardised scenarios. Within this comparison, mode~2 showed the clearest inverse association with SIJ motion, especially SIJ translation in stance-related loading, whereas mode~9 was linked to SIJ motion more weakly and less consistently.

Functionally, the data therefore argue against diffuse female laxity and instead support mode-specific reorganisation. Both sexes retained a largely shared low-order support backbone in bilateral and unilateral stance, consistent with the continuing mechanical priority of habitual load transfer \cite{ref_current}. Sex differences were concentrated in a restricted subset of pathways: mode~2 shows that reorganisation reaches into the baseline support architecture, whereas mode~9 identifies a childbirth-relevant inlet-opening route whose sex bias cannot be reduced to canal dimensions alone \cite{ref_current}. The sensitivity results sharpen this distinction. At the whole-ring level, mode~9 remained strongly regulated by the SIJ cartilage interfaces together with the pubic symphysis, yet the sex difference in this mode was not explained by disproportionately greater female SIJ-cartilage sensitivity; instead, SIJ-cartilage sensitivities were broadly similar between sexes, while pubic symphysis sensitivity for modes~2 and~9 was higher in males. The conservative evolutionary interpretation is therefore not that one sex was selected for maximal stiffness and the other for general softness, but that pelvic mechanics preserves a shared support architecture while reallocating compliance in selected pathways so as to remain compatible with both everyday force transmission and obstetric accessibility \cite{ref_current,ref_sensitivity,ref_SIJ_review,ref_SIJ_sex}.


A useful way to frame this result is that the raw size effect is itself informative. Because female pelves are smaller on average, elementary structural scaling predicts higher, not lower, eigenvalues if sex differences were purely geometric. That expectation was met for most modes in the unscaled spectrum, but not for all of them. Modes~2 and~9 were already more accessible in women before size correction, despite the opposing size trend. They are therefore not merely the modes that remain significant after statistical adjustment; they are the modes in which sex-specific organisation is strong enough to overcome the baseline stiffening expected for a smaller pelvic ring.

Functionally, this argues against the idea of a generally more compliant or lax female pelvis. A globally softened ring would be expected to shift the spectrum in the same direction across most modes. That is not what we observed. Instead, most modes remain consistent with the expected stiffness of a smaller structure, while only a restricted subset of low-order pathways is selectively more accessible in women. In mechanical terms, this pattern is more consistent with targeted redistribution of compliance than with diffuse loosening.
