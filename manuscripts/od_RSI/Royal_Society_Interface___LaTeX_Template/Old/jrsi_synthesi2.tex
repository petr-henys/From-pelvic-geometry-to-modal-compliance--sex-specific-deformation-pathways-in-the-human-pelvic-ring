...
That framing remains useful, but it is incomplete. The pelvis is not merely a set of distances. It functions as a deformable structure whose performance depends on the available deformation pathways, the extent to which they are recruited by a given load, and the anatomical constraints that regulate access to them. Multiple lines of research have challenged the notion that pelvic design is a zero-sum compromise between locomotion and childbirth. Biomechanical studies indicate that moderately wider hips do not incur a locomotor penalty: women with broader pelvises walk and run just as efficiently as those with narrow hips \cite{Warrener2015}. In parallel, evolutionary biologists have proposed that metabolic factors could set the limits on human childbirth \cite{Dunsworth2012}, and obstetrics shifted to research on pelvic floor muscles and connective tissues \cite{Pavlicev2020}. Clinical experience in general has reached a comparable conclusion: pelvic form matters, but static pelvic measurements alone rarely predict labour outcomes \cite{bergella}. Yet the importance of pelvic anatomy did not disappear, raising the possibility that pelvic form harbours “hidden” mechanical features, invisible to static measurements.

Over the past few decades, advances in pelvic biomechanics have set a firm ground for this research by incorporating real anatomical detail into simulations. We already have subject-specific finite element (FE) models that include heterogeneous bone material properties as well as cartilage and ligaments \cite{Eichenseer2011,Hammer2013}. However, what remains limited is a population-scale framework that directly describes pelvic mechanics. The isolated subject-specific models do not readily reveal which features of mechanical behaviour are shared across individuals, which vary with scale, and which are organised by sex. Here, we address that gap using correspondence-preserving modal analysis of 278 anatomically standardised human pelves. Modal decomposition treats the pelvis not as a rigid set of obstetric diameters but as a repertoire of deformation pathways with different stiffness costs. This shift separates two distinct questions. First, which pathways are mechanically accessible? Second, once a pathway is available, how is the response organised under a given load? That distinction is central to the mechanics of childbirth. A pelvis may permit access to a pathway without concentrating its response into that pathway, and a mechanically important sex difference may lie in response organisation rather than in gross widening or global softening.
...
Allometric regressions showed near-isometric scaling of total volume with pelvic scale (slope $=3.000$, $R^2=1.000$), shallower scaling of total surface area (slope $=1.684$, $R^2=0.894$), increasing total mass with scale (slope $=2.679$, $R^2=0.902$), and a modest decline in effective density with scale (slope $=-0.321$, $R^2=0.117$; table~\ref{tab:allometry}). 

\subsection{Modal analysis, Size effect and Static morphometry}

Finite-element modelling showed that pelvic size explained an important part of baseline stiffness variation and, by implication, much of the accessibility of deformation pathways.
However, two modes already departed from the expected raw size trend: women showed lower unscaled eigenvalues than men in modes~2 and~9 despite smaller average pelvic size. These reversals were especially informative, because they indicated that sex-specific mechanical organisation was already strong enough to overcome the baseline stiffening expected for a smaller ring.
...
\subsection{Soft tissue sensitivity to selected modes}
...
The limited explanatory power of conventional pelvimetry reinforces this point. In the female subset, no single static dimension explained more than a modest proportion of the variance in the size-corrected eigenvalues of the mechanically relevant modes. PIT and AP retained some predictive value, but the explained variance remained low. Static dimensions therefore capture only part of the functional phenotype. This does not diminish the anatomical importance of pelvimetric variables; rather, it places them in context. A pelvic inlet may be wider or narrower, but the mechanical significance of that geometry depends on how it is embedded within the full ring, including bone distribution, cartilage interfaces and ligamentous constraints. From a functional perspective, clinically familiar dimensions are better understood as partial correlates of a broader mechanical architecture than as direct surrogates for it.

The loading analyses further clarify which elements of that architecture are shared between the sexes and which are selectively reorganised. Under bilateral and unilateral stance, both sexes relied on a largely conserved low-order support basis. Bilateral stance was dominated by mode~2 in both women and men, whereas unilateral stance distributed response mainly across modes~1--3, with only minor sex differences in participation fractions. These findings indicate that habitual support and locomotor load transfer are built on a common mechanical backbone. In other words, the data do not support a view in which the female pelvis achieves childbirth-related function by sacrificing the basic organisation required for everyday support. Instead, the support-related repertoire appears broadly conserved.

The childbirth-motivated load cases revealed a different pattern. Under simulated inlet opening, a single modal component dominated in the great majority of subjects, making it the clearest candidate for the principal inlet-opening pathway in the present framework. Compared with habitual loading, however, the sexes differed in how the response was distributed around this principal pathway. Men concentrated a greater proportion of the inlet-opening response in mode~9 itself, whereas women recruited a broader set of neighbouring low-order modes, particularly mode~4, while still retaining mode~9 as the dominant pathway. This distinction is mechanistically important. It suggests that female pelvic response is not characterised by simple amplification of a single obstetrically relevant mode, but by a more distributed low-order compliance pattern. The same general class of deformation is accessed in both sexes, but the organisation of participation differs.

A related pattern was observed for outlet opening. In both sexes, mode~5 emerged as the dominant outlet-related pathway, yet women retained greater concurrent participation of mode~9, whereas men showed relatively greater contribution of mode~6. This suggests that inlet- and outlet-related mechanics are not independent modules, but overlapping expressions of the same ring system. The low-order repertoire appears to be reused across functional scenarios, with changes in weighting rather than wholesale replacement of one mechanical programme by another. This observation fits the broader interpretation of the pelvis as a coupled structure in which childbirth-related behaviour is superimposed on, rather than separated from, support-related mechanics.

The sensitivity analyses provide important complementary evidence. Although limited to the template-based framework, they identify which components of the model exert the strongest control over the mechanically relevant pathways. In the inlet-opening scenario, the highest sensitivities of the dominant mode were associated with the sacroiliac cartilage interfaces, the pubic symphysis interface, the symphyseal ligament group and the anterior sacroiliac ligaments, whereas posterior ligament groups contributed much less. The regulatory capacity of pelvic soft tissues is already well recognised; what the present results add is evidence that this regulation is mode-selective and may provide a mechanism for transiently modulating access to specific deformation pathways.
